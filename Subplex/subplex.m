function [xbest, xbestBound, fx, num_eval, info]=...
...
%
eval_string = ' ';
call_string = '[cur_x,cur_fx,num_eval,flag,last_prt]=simplex(func,nx,step_size,ss_dimen,order(whichx:(whichx+ss_dimen-1)),cur_x, num_eval, pass, last_prt,eval_string';
if (nargin < 2)
...
num_eval = 1;
cur_x = startx;
% pass =[alpha,beta,gamma,delta,psi,do_print,fx_tsop,max_eval];
...
xbest = cur_x;                      % best parameters (not bounded)
xbestBound = BoundParams(cur_x);   % best parameters  bounded)
fx = cur_fx;