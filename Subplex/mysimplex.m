function [minx, fminx, num_eval, flag, last_prt] =  ...
	mysimplex(nx,scalev,ss_dim,setorder,minx,num_eval,...
		pass,last_prt,p1,p2,p3,p4,p5,p6,p7,p8,p9,p10)
%
...
%
	for i = 1:numpoints
        ...
        fxset(i)=my_model((usex+simp_xvals));
		num_eval = num_eval+1;
	end
%
...
%
	   ...
	   if(~coinc_test)
          ...
          new_fx=my_model((usex+simp_xvals));
	      num_eval=num_eval+1;
...
%
		 ...
		 if (~coinc_test)
             ...
             exp_fx=my_model((usex+simp_xvals));
		     num_eval = num_eval+1;
		     if (exp_fx < new_fx)
		        fxset(ind_hi) = exp_fx;
		     else
...
%
            ...
            contr_fx=my_model((usex+simp_xvals));
		    num_eval=num_eval+1;
			  ...
			  if (index ~= ind_low)
			     tempvec = simplx_sets(:,index);
				   ...
				   delta* (simplx_sets(:,ind_low)-tempvec);
                    ...
                    fxset(index)=my_model((usex+simp_xvals));
			        num_eval= num_eval+1;
		             end
			  end
			     index = index + 1;
