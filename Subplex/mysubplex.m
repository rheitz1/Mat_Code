...
%	get an initial point for func
	cur_fx = my_model(startx);
	%eval_string = my_model((usex+simp_xvals));
	num_eval = 1;
        cur_x = startx;
...
%
			[cur_x,cur_fx,num_eval,flag,last_prt]=mysimplex(nx,step_size,ss_dimen,order(whichx:(whichx+ss_dimen-1)),cur_x, num_eval, pass, last_prt);
			if (flag)
				tflag = TRUE;
				info = flag;
			end
