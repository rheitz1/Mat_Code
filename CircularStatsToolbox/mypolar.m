...
%MYPOLAR  Polar coordinate plot.
%   MYPOLAR(THETA, RHO) makes a plot using polar coordinates of
%   the angle THETA, in radians, versus the radius RHO.
%   MYPOLAR(THETA,RHO,S) uses the linestyle specified in string S.
%   See PLOT for a description of legal linestyles.
%   MYPOLAR(THETA,RHO,S,SCALE) sets the scale of the outer circle
%   (solid line) to SCALE and the inner circle (dashed line) to
%   1/2 SCALE.  Default is 1.  Added by BP 6.7.2011.
%
%   MYPOLAR(AX,...) plots into AX instead of GCA.
%
%   H = MYPOLAR(...) returns a handle to the plotted object in H.
%
%   Example:
%      t = 0:.01:2*pi;
...
%force cax limits to 1
% set(cax,'xlim',[0 1],'ylim',[0 1])
...
if ~hold_state
    ...
    maxrho=scale;%BP - force limits to 1
    set(cax,'ylim',[-maxrho maxrho],'xlim',[-maxrho maxrho])
    ...
    rt = 1.3*rmax;
    for i = 1:4%length(th)
        text(rt*cst(i),rt*snt(i),int2str(i*45),...
            'horizontalalignment','center',...
            'handlevisibility','off','parent',cax);
        if i == 4
            loc = int2str(0);
        else
            loc = int2str(180+i*45);
        end
