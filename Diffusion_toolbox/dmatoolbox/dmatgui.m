...
%   analysis.
%
%   DMATGUI makes use of the undocumented (and thus unsupported) function
%   UITABLE, for which some functionality may change with newer versions of
%   MATLAB. DMATGUI was written and tested on:
...
%
%   If you have a more recent version of MATLAB and DMATGUI (in particular,
%   the tables and spreadsheets) exhibits unexpected behavior, check the
%   website for updates.
%
%   See also MULTIESTV4.
%
...
%
    function OpeningFcn
        ...
        % Trap annoying UITABLE message with EVALC
        evalc(['DMAT.GUIcontrols.MainWindow.tbl=uitable(',...
            'DMAT.GUIcontrols.WindowHandles.Main,cell(80,3),',...
            '{''Condition'',''Response'',''Reaction Time''},',...
            '''Position'',[.5 .5 .5 .5]);']);
        set(DMAT.GUIcontrols.MainWindow.tbl,'Editable',0,...
            'Units','normalized',...
            ...
            if ~isempty(button)
                switch button(1)
                    case 'Y'
                        gc = DMAT.GUIcontrols;
                        dmt = [];
                        % Trap annoying error message from UITABLE
                        evalc('dmt = load(''~$crash.dmt'',''-mat'');');
                        DMAT = dmt.DMAT;
                        DMAT.GUIcontrols = gc;
                        setstats
...
%
    function biasbut_Callback
        ...
        if ~isfield(DMAT.Data,'NCond')
            ...
            DMAT.GUIcontrols.BiasWindow.tbl = [];
            % UITABLE message...
            evalc(['DMAT.GUIcontrols.BiasWindow.tbl = uitable(',...
                'DMAT.GUIcontrols.WindowHandles.Bias,',...
                'sho,title,',...
                '''Position'',[20 45 150 60],',...
                '''ColumnWidth'',50);']);
            set(DMAT.GUIcontrols.BiasWindow.tbl,...
                'Units','normalized',...
                'Position',[.06 .4 .88 .5])
    ...
    function desp_Callback
        ...
        elseif DMAT.Data.NCond<=1
            ...
            if val
                ...
                cols = size(sho,2);
                data = [num2cell(sho) ...
                    cell(DMAT.Data.NCond,DMAT.Data.NCond-cols)]; %#ok
                labels = [num2cell(num2str((1:cols)')'),...
                    repmat({'.'},1,DMAT.Data.NCond-cols)]; %#ok
                % UITABLE message...
                DMAT.GUIcontrols.DesignWindow.tbl = [];
                evalc(['DMAT.GUIcontrols.DesignWindow.tbl = uitable(',...
                    'DMAT.GUIcontrols.WindowHandles.Design,',...
                    'data,',...
                    'labels,',...
                    '''Position'',[1 1 1 1],',...
                    '''ColumnWidth'',35);']);
                set(DMAT.GUIcontrols.DesignWindow.tbl,...
                    'units','normalized',...
    ...
    function fixin_Callback
        ...
        if ~isfield(DMAT.Data,'NCond')
            ...
            if isfield(DMAT.CurrentModel,'FixedValues')
                ...
                sho = reshape(sho,DMAT.Data.NCond,7); %#ok
            end 
            % UITABLE message...
            DMAT.GUIcontrols.FixesWindow.tbl = [];
            evalc(['DMAT.GUIcontrols.FixesWindow.tbl = uitable(',...
                'DMAT.GUIcontrols.WindowHandles.Fixes,',...
                'sho,',...
                'param,',...
                '''Position'',[1 1 1 1],',...
                '''ColumnWidth'',35);']);
            set(DMAT.GUIcontrols.FixesWindow.tbl,...
                'units','normalized',...
                'Position',[.06 .3 .86 .5]);
            set(DMAT.GUIcontrols.FixesWindow.tbl,'data',sho)
        ...
        if (nmods>0 || otype<3) && ...
            ...
            switch otype
                case 1
                    ...
                    evalc(['DMAT.GUIcontrols.OutputWindow.descrip = uitable(',...
                        'DMAT.GUIcontrols.WindowHandles.Output,',...
                        '''data'',descriptives(DMAT.Data.Dataset),',...
                        '''columnnames'',{'' '','' '','' '','' ''},',...
                        '''ColumnWidth'',100,',...
                        '''Position'',[100 100 100 100]);']);
                    set(DMAT.GUIcontrols.OutputWindow.descrip,...
                        'Units','normalized','Position',[.32 .19 .615 .67]);
                ...
                case 3
                    ...
                    minia = reshape(cellstr(num2str(minia(:),'%8.5f')),...
                        size(minia)); %#ok
                    evalc(['DMAT.GUIcontrols.OutputWindow.parest = uitable(',...
                        'DMAT.GUIcontrols.WindowHandles.Output,',...
                        '''data'',minia,',...
                        '''columnnames'',columnlabels,',...
                        '''ColumnWidth'',60,',...
                        '''Position'',[1 1 1 1]);']);
                    set(DMAT.GUIcontrols.OutputWindow.parest,...
                        'Units','normalized','Position',[.32 .19 .615 .67]);
                    DMAT.GUIcontrols.OutputWindow.current = ...
                ...
                case 4
                    ...
                    stds = reshape(cellstr(num2str(stds(:),'%8.5f')),size(stds)); %#ok
                    evalc(['DMAT.GUIcontrols.OutputWindow.stderr = uitable(',...
                        'DMAT.GUIcontrols.WindowHandles.Output,',...
                        '''data'',stds,',...
                        '''columnnames'',columnlabels,',...
                        '''ColumnWidth'',60,',...
                        '''Position'',[1 1 1 1]);']);
                    set(DMAT.GUIcontrols.OutputWindow.stderr,...
                        'Units','normalized','Position',[.32 .19 .615 .67]);
                    DMAT.GUIcontrols.OutputWindow.current = ...
                ...
                case 7
                    ...
                    hessy = reshape(cellstr(num2str(hessy(:),'%8.5f')),...
                        size(hessy)); %#ok
                    cn = namepars(DMAT.GUIcontrols.OutputWindow.thismod,1); %#ok
                    evalc(['DMAT.GUIcontrols.OutputWindow.hessian = uitable(',...
                        'DMAT.GUIcontrols.WindowHandles.Output,',...
                        '''data'',hessy,',...
                        '''columnnames'',cn'',',...
                        '''Position'',[1 1 1 1]);']);
                    set(DMAT.GUIcontrols.OutputWindow.hessian,...
                        'Units','normalized','Position',[.32 .19 .615 .67]);
                    DMAT.GUIcontrols.OutputWindow.current = ...
...
%
    function modelfitstats
        ...
        evalc(['DMAT.GUIcontrols.OutputWindow.models = uitable(',...
            'DMAT.GUIcontrols.WindowHandles.Output,',...
            '''data'',[mfs;cell(1,6)],',...
            '''columnnames'',{''Model'',''Dev'',''df'',''AICc'',''BIC'',''LR''},',...
            '''Position'',[1 1 1 1]);']);
        set(DMAT.GUIcontrols.OutputWindow.models,...
            'Units','normalized',...
...
%
    function outliertreatmentreport(method)
        ...
        function showmms
            ...
            showcutoff
            sho = [pis gammas];
            sho = reshape(cellstr(num2str(sho(:),'%8.5f')),size(sho));%#ok
            evalc(['DMAT.GUIcontrols.OutputWindow.mixpars = uitable(',...
                'DMAT.GUIcontrols.WindowHandles.Output,',...
                '''data'',[sho;cell(1,2)],',...
                '''columnnames'',{''pi'',''gamma''},',...
                '''Position'',[1 1 1 1]);']);
            set(DMAT.GUIcontrols.OutputWindow.mixpars,...
                'Units','normalized',...
                'Position',[.32 .19 .5 .6]);
            DMAT.GUIcontrols.OutputWindow.current = ...
                [DMAT.GUIcontrols.OutputWindow.current;
...
%
    function guess_Callback
        ...
        if get(DMAT.GUIcontrols.AdvancedWindow.guess,'Value')==3
            ...
            evalc(['DMAT.GUIcontrols.GuessWindow.partbl = uitable(',...
                'DMAT.GUIcontrols.WindowHandles.Guess,cell(r,c),',...
                '{''a'', ''Ter'', ''eta'', ''z'', ''sZ'', ''st'', ''v''},',...
                '''ColumnWidth'',42,',...
                '''Position'',[1 1 1 1]);']);

            set(DMAT.GUIcontrols.GuessWindow.partbl,...
                'units','normalized',...
                'Position',[.05 .28 .9 .6]);
            set(DMAT.GUIcontrols.GuessWindow.partbl,'Data',cl);
    ...
    function storeguess_Callback
        ...
        if any(any(~cellfun('length',d)))
            ...
            if ~b
                ...
                evalc(['DMAT.GUIcontrols.GuessWindow.partbl = uitable(',...
                    'DMAT.GUIcontrols.WindowHandles.Guess,cell(size(d)),',...
                    '{''a'', ''Ter'', ''eta'', ''z'', ''sZ'', ''st'', ''v''},',...
                    '''Data'',d,',...
                    '''ColumnWidth'',42,',...
                    '''Position'',[1 1 1 1]);']);
...
%
    function tools_Callback
        ...
        evalc(['DMAT.GUIcontrols.ToolsWindow.partbl = uitable(',...
            'DMAT.GUIcontrols.WindowHandles.Tools,cell(r,c+1),',...
            '{''a'', ''Ter'', ''eta'', ''z'', ''sZ'', ''st'', ''v'', ''N''},',...
            '''ColumnWidth'',35,',...
            '''Position'',[1 1 1 1]);']);

        set(DMAT.GUIcontrols.ToolsWindow.partbl,...
            'units','normalized',...
            'Position',[.05 .28 .9 .6]);
        set(DMAT.GUIcontrols.ToolsWindow.partbl,'Data',cl);
    ...
    function simulsave_Callback
        ...
        if any(any(~cellfun('length',d)))
            ...
            if ~b
                ...
                evalc(['DMAT.GUIcontrols.ToolsWindow.partbl = uitable(',...
                    'DMAT.GUIcontrols.WindowHandles.Tools,cell(size(d)),',...
                    '{''a'', ''Ter'', ''eta'', ''z'', ''sZ'', ''st'', ''v'', ''N''},',...
                    '''Data'',d,',...
                    '''ColumnWidth'',35,',...
                    '''Position'',[1 1 1 1]);']);
