function [coh,f,tout,Sx,Sy,Pcoh,PSx,PSy,N,dn,nwin,err,tmpdf1,tmpdf2] = Spk_SpkCoh(Spk,Spk2,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts)
%   [coh,f,tout,Sx,Sy,Pcoh,PSx,PSy,N,dn,nwin,err,tmpdf1,tmpdf2] = Spk_SpkCoh(Spk,Spk2,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts)
%                   or
...
%                   padded with NaNs.
%       Spk2    =   Second spike train, in same format as Spk 
...
%        SAMPLING 	=  Sampling rate woth which to use for the Continuous 
%                   Spike time series, that will be the Fs for the FFT used 
%                   Defaults to 1000 Hz (Apt for spikes)
...
%
%  Outputs from tfspec:
%       COH	=  Coherency of Spk and Spk2 in [Time, Freq] form if flag is 1 (the 
...
%                   the phase of Spk2. So an angle b/t 0 and Pi means Spk 
%                   leads X, an angle b/t Pi and 2Pi means X leads Spk.
%
...
%       tout    =  Vector of values for time axis for COH. NOTE- this
...
%       Sy      =  The spectrum of the signal associated with Spk2, the second
...
normfreqs=0;    %to adjust teh vals of spec if plotting by normalizing each freq across time... this makes it impossible to compare across freqs, but allows visualization of less dominant freqs simultaneously with more dominant freqs
...
if nargin<7 || isempty(fk);  fk = [0,200]; end
if nargin<6 || isempty(dn); dn=.01; end %In seconds. 
%if nargin < 4 dn = n/10; end % Bijan's default within tfspec, where n is the length of the tapers in seconds
if nargin<5 || isempty(sampling);   sampling=1000; end
if nargin < 4 || isempty(bn);  bn=[min(min( Spk )) max(max( Spk ))]; end
...
if nargout > 5 && JustPartFlag;
    disp('In Spk_Spk Coh., calcing just the partial coh, given in first output (coh). 6th output (PCoh) will be empty.')
    Pcoh=[];
end

if CalcSpec
    if PSDScaleFlag;     estStr='PSD';      else    estStr='MS Spectrum';      end
    %disp(['In Spk_Spk Coh., and will calc spec also. Spec output units will be: ' estStr])
end

%instead of having a tfSpkSpec script, we'll include that part of it below
ContSpk=mySpk2Cont(Spk, bn, sampling);
ContSpk2=mySpk2Cont(Spk2, bn, sampling);
% params.tapers=[1 1];
...
if (errorchk && errType==1) || ZTransFlag  || nargout>12
    ...
    NTotSpksPerWin2=NTotSpksPerWin;
else
    RecNSpksFlag=0;
end
...
for win = 1:nwin
    ...
    %tmp = ContSpk(:,(win-1)*dn+1:(win-1)*dn+N)';   %changed to win-1 becasue that ignored the first few samples, also added 1 to nwin above
    %tmp2 = ContSpk2(:,(win-1)*dn+1:(win-1)*dn+N)';
    ...
    dat2 = ContSpk2(:,(win-1)*dn+1:(win-1)*dn+N);
    %dmtSSCoh_S
    dmtUbiq_S
end
