function [Sx,f,tout,PSx,N,dn,nwin,err,tmpdf1] = SpkSpec(Spk,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts,normfreqs)
%   [Sx,f,tout,PSx,N,dn,nwin,err,tmpdf1] = SpkSpec(Spk,tapers,bn, sampling, dn, fk, plotflag, pad, pval, JustPartFlag,errType,ZTransFlag,ClusShuffOpts)
%
...
%                   uoutput the mean-square spectrum, in units of mV^2.
...
%normfreqs=0;    %to adjust teh vals of Sx if plotting by normalizing each freq across time... this makes it impossible to compare across freqs, but allows visualization of less dominant freqs simultaneously with more dominant freqs
...
if nargin < 14 || isempty(normfreqs); normfreqs = 0; end
if nargin<13 || isempty(ClusShuffOpts);         ClusShuffOpts=[];      end

if nargin < 12 || isempty(ZTransFlag);    ZTransFlag=0;     end
if nargin < 11 || isempty(errType);    errType=2;     end
if nargin < 10 || isempty(JustPartFlag);    JustPartFlag=0;     end
%if nargin < 10 || isempty(flag); flag = 1; end
if nargin < 9 || isempty(pval); pval = 0.05; end
if nargin < 8 || isempty(pad); pad = 1; end
...
if nargin < 3 || isempty(bn);  bn=[min(min( Spk )) max(max( Spk ))]; end
...
disp(['In Spk spec output units will be: ' estStr])
...
if (errorchk && errType==1) || ZTransFlag  || nargout>8
    RecNSpksFlag=1;
