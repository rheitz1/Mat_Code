function S = json_encode(data,tp,depth)
      ...
      nI = size(data,1);
        ...
        if tp=='h',
          ...
          S = [S '"' key '":'];
				...
				pf = data{i,1}(1);
				S = [S json_encode(data{i,2},pf,depth+1)];
				if i<nI, S(end+1) = ',';  end
				if indent, S(end+1) = sprintf('\n'); end
			end
  ...
  case 'm', % matrix, numerical values
    if isnumeric(data) && numel(size(data)) == 2,
   		S = sprintf('[\n');
			nI = size(data,1);
			for i=1:nI,
    	  S(end+1:end+2*depth+2) = ' '; % indent
				v = data(i,:);
				q = json_encode(v,'v',depth+1);
				S(end+1:end+numel(q)) = q;
				if i<nI, S(end+1) = ',';  end
				S(end+1) = sprintf('\n');
			end
			S(end+1:end+2*depth) = ' '; S(end+1) = ']';
    else
