function [data,tp] = json_decode(S,simplify)
% [DATA,TP] = json_decode(S,SIMPLIFY)
%
...
%
% If SIMPLIFY is set to TRUE (the default), json_decode simplifies the 
...
% - if all fieldnames of a json object contain nothing but letters, 
%   numbers and underscores, the cell is converted to a struct,
%   with fieldnames prefixed by data type and underscore.
%
% json_decode is invertible, in that [S] = json_encode(data,tp)
% reconstructs S (formatting may differ).
%
% [TP_DATA] = json_decode(S,SIMPLIFY)
% 
