...
sz = eval('sz','''M''');
baseUrl = 'http://127.0.0.1/incf/scalablebrainatlas/';
...
tmp = urlread([baseUrl 'mfiles/get_scaling.php'],'get',{'template',template});
scaling = eval(char(tmp));
slices = scaling.sliceRange(1):scaling.sliceRange(3):scaling.sliceRange(2);
numSlices = numel(slices);

% download the rasterized images and store them in V
fprintf('Loading %d slices...     ',numel(slices));
for si=1:numSlices,
  fprintf('\b\b\b\b%04d',si);
  [tmp,map] = imread([baseUrl 'rgbslice.php?template=' template '&slice=' num2str(si-1) '&size=' sz]); 
  if numel(size(tmp)) == 3,
