...
% returns the change in eye position. 
...
%try to get SRT_end variable.  Due to recursion, just throw a message
%rather than try to call getSRT
try
    SRT_end = evalin('caller','SRT_end');
catch
    disp('SRT_end variable not present')
    return
end
...
SRT = round(SRT);
SRT_end = round(SRT_end);

%get saccade ending times
...
%Preallocate
pos.X(1:size(EyeX_,1),1:100) = NaN;
pos.Y(1:size(EyeX_,1),1:100) = NaN;

%==========================================================
% Get eye movement information
% For locations, take 10 ms before saccade to 10 ms after saccade
for trl = 1:size(EyeX_,1)
    if ~isnan(SRT(trl,1)) & SRT(trl,1) < 2000 & SRT(trl,1) > 50
        
        
        eyeWin = 5; %number of ms to average over
        offset = 20; %how many ms to move into past (SRT) or future (SRT_end) to make sure eye not still moving?
        %for eye position traces, use x ms before saccade to x ms after saccade
         pos.X(trl,1:length(SRT(trl,1)-eyeWin:SRT_end(trl,1)+eyeWin)) = EyeX_(trl,SRT(trl,1) + Target_(1,1) - eyeWin:SRT_end(trl,1) + Target_(1,1) + eyeWin);
         pos.Y(trl,1:length(SRT(trl,1)-eyeWin:SRT_end(trl,1)+eyeWin)) = EyeY_(trl,SRT(trl,1) + Target_(1,1) - eyeWin:SRT_end(trl,1) + Target_(1,1) + eyeWin);
         %pos.X(trl,1:length(SRT(trl,1)-10:SRT(trl,1)+10)) = EyeX_(trl,SRT(trl,1) + Target_(1,1) - 10:SRT(trl,1) + Target_(1,1) + 10);
         %pos.Y(trl,1:length(SRT(trl,1)-10:SRT(trl,1)+10)) = EyeY_(trl,SRT(trl,1) + Target_(1,1) - 10:SRT(trl,1) + Target_(1,1) + 10);
        
        %change in eye position will be x ms averages around saccade start and end
        loc_start_X = mean(EyeX_(trl,Target_(1,1) + SRT(trl,1) - offset - eyeWin:Target_(1,1) + SRT(trl,1) - offset));
        loc_start_Y = mean(EyeY_(trl,Target_(1,1) + SRT(trl,1) - offset - eyeWin:Target_(1,1) + SRT(trl,1) - offset));
        
        %for end point, shift x ms into the future to make sure eyes are not still moving
        loc_end_X = mean(EyeX_(trl,SRT_end(trl,1) + Target_(1,1) + offset:SRT_end(trl,1) + Target_(1,1) + offset + eyeWin));
        loc_end_Y = mean(EyeY_(trl,SRT_end(trl,1) + Target_(1,1) + offset:SRT_end(trl,1) + Target_(1,1) + offset + eyeWin));
...
for var = 1:nVar
    ...
    % TEMPORARY - DO NOT CONVERT TO DEGREES
   % position.convX = 1;
   % position.convY = 1;
