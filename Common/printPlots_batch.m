...
batch_list = dir('C:\Data\ToTranslate\*.mat');
q='''';c=',';qcq=[q c q];
...
for i = 1:length(batch_list)
    ...
    varlist = who('-file',batch_list(i).name);
    ...
    for c = 1:length(cells)
        ...
        eval(['load(' q,file_path,batch_list(i).name,qcq,cell2mat(cells(c,:)),qcq,'-mat',q,')'])
    end

    %load Target_,Correct_,SRT
    eval(['load(' q,file_path,batch_list(i).name,qcq, 'Decide_' qcq 'EyeX_' qcq 'EyeY_' qcq 'newfile' qcq 'Target_' qcq 'Correct_' qcq 'Errors_' qcq 'TrialStart_' qcq,'-mat',q,')'])
    ...
    keep i batch_list printflag saveflag maximizeflag q c qcq
end
