...
function [tuning] = getTuning_SP_targ(Spike,interval,freqrange,plotFlag)
...
SDF0 = spikedensityfunct(Spike,Target_(:,1),interval,pos0,TrialStart_);
SDF1 = spikedensityfunct(Spike,Target_(:,1),interval,pos1,TrialStart_);
SDF2 = spikedensityfunct(Spike,Target_(:,1),interval,pos2,TrialStart_);
SDF3 = spikedensityfunct(Spike,Target_(:,1),interval,pos3,TrialStart_);
SDF4 = spikedensityfunct(Spike,Target_(:,1),interval,pos4,TrialStart_);
SDF5 = spikedensityfunct(Spike,Target_(:,1),interval,pos5,TrialStart_);
SDF6 = spikedensityfunct(Spike,Target_(:,1),interval,pos6,TrialStart_);
SDF7 = spikedensityfunct(Spike,Target_(:,1),interval,pos7,TrialStart_);
...
if plotFlag == 1
    ...
    subplot(2,2,2)
    plot(0:7,tuning.minmax,'-ob',0:7,tuning.max,'-or',0:7,tuning.min,'-ok')
    xlim([0 7])
    ...
    subplot(2,2,4)
    plot(0:7,tuning.fft,'-ok','markerfacecolor','k')
    xlim([-.5 7.5])
    title('FFT Tuning')
