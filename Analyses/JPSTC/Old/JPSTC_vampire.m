...
for pair = 1:size(pairings,1)
    ...
    %use -50 to 400
    sig1_small = sig1(:,450:900);
    sig2_small = sig2(:,450:900);
    plottime_small = (-50:400);
    plottime = (-500:2500);
    ...
    % preallocate space
    cor_small(1:size(sig1_small,1),1:401) = NaN;
    ...
    for trl = 1:size(sig1,1)
        h = trl/size(sig1,1)
        cor_small(trl,1:401) = xcorr(sig2(trl,:),sig1(trl,:),200,'coeff');
...
% % % %     JPSTC_matrix_small(1:451,1:451) = NaN;
% % % %     JPSTC_matrix(1:3001,1:3001) = NaN;


%use times -50 to 400
    for time1 = 1:size(sig1_small,2)
        h = time1/size(sig1_small,2)
        for time2 = 1:size(sig2_small,2)
            JPSTC_matrix_small(time1,time2) = corr(sig1_small(:,time1),sig2_small(:,time2));
            shift_predictor_small(time1,time2) = corr(circshift(sig1_small(:,time1),1),sig2_small(:,time2));
        end
    end
 

    %use times -500 to 2500
...
%     end
    ...
    if pdfFlag == 1
        ...
        eval(['print -dpdf ',q,'~/Output/',file,'_',[cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'.pdf',q])
%         eval(['print -dpdf ',q,outdir,cell2mat(file_name(file)),'_',[cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'.pdf',q]);
    end
