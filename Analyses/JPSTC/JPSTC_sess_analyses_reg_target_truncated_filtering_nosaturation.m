...
cd /volumes/dump2/JPSTC/JPSTC_matrices/reg/correct_target_truncated_filtered_nosaturation/LFP-LFP/
...
JPSTC_correct_notch_all(1:451,1:451,1:length(batch_list)) = NaN;
JPSTC_correct_band_all(1:451,1:451,1:length(batch_list)) = NaN;
cor_correct_all(1:length(batch_list),1:401) = NaN;
cor_correct_notch_all(1:length(batch_list),1:401) = NaN;
cor_correct_band_all(1:length(batch_list),1:401) = NaN;
...
for i = 1:length(batch_list)
    i
    load(batch_list(i).name,'JPSTC_correct','JPSTC_correct_band','JPSTC_correct_notch','cor_correct','cor_correct_band','cor_correct_notch')
...
%     clear thin_temp
    ...
    cor_correct_notch_all(i,1:401) = nanmean(cor_correct_notch);
    cor_correct_band_all(i,1:401) = nanmean(cor_correct_band);

    %mean JPSTC
    JPSTC_correct_all(1:451,1:451,i) = JPSTC_correct;
    JPSTC_correct_notch_all(1:451,1:451,i) = JPSTC_correct_notch;
    JPSTC_correct_band_all(1:451,1:451,i) = JPSTC_correct_band;
    
    clear JPSTC_correct JPSTC_correct_notch JPSTC_correct_band ...
        cor_correct cor_correct_notch cor_correct_band
end
clear batch_list i
