...
function [] = Search_Errors_n2pc_subsample(file)
f = figure;
q = '''';
c = ',';
qcq = [q c q];
...
if isempty(DSPlist)
    ...
    for DSPchan = 1:size(DSPlist,1)
        ...
        in_correct = shake(find(Correct_(:,2) == 1 & ismember(Target_(:,2),RF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
        out_correct = shake(find(Correct_(:,2) == 1 & ismember(Target_(:,2),antiRF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
        in_incorrect = shake(find(ismember(Target_(:,2),RF) & ismember(SaccDir_,antiRF)));
        out_incorrect = shake(find(ismember(Target_(:,2),antiRF) & ismember(SaccDir_,RF)));
        
        newlength = findMin(length(in_correct),length(out_correct),length(in_incorrect),length(out_incorrect));
        
        in_correct = in_correct(randperm(newlength));
        out_correct = out_correct(randperm(newlength));
        in_incorrect = in_incorrect(randperm(newlength));
        out_incorrect = out_incorrect(randperm(newlength));
...
%     OR = filtSig(OR,40,'lowpass');
    ...
    contraCorrectOL = shake(find(~isnan(OL(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),[7 0 1]) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsiCorrectOL = shake(find(~isnan(OL(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),[3 4 5]) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50));
    
    contraCorrectOR = shake(find(~isnan(OR(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),[3 4 5]) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsiCorrectOR = shake(find(~isnan(OR(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),[7 0 1]) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50));
    
    
    contraErrorsOL = shake(find(~isnan(OL(:,1)) & ismember(SaccDir_,[3 4 5]) & ismember(Target_(:,2),[7 0 1]) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsiErrorsOL = shake(find(~isnan(OL(:,1)) & ismember(SaccDir_,[7 0 1]) & ismember(Target_(:,2),[3 4 5]) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    
    contraErrorsOR = shake(find(~isnan(OR(:,1)) & ismember(SaccDir_,[7 0 1]) & ismember(Target_(:,2),[3 4 5]) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsiErrorsOR = shake(find(~isnan(OR(:,1)) & ismember(SaccDir_,[3 4 5]) & ismember(Target_(:,2),[7 0 1]) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    
    newlengthOL = findMin(length(contraCorrectOL),length(ipsiCorrectOL),length(contraErrorsOL),length(ipsiErrorsOL));
    newlengthOR = findMin(length(contraCorrectOR),length(ipsiCorrectOR),length(contraErrorsOR),length(ipsiErrorsOR));
    
        
    %equate trial numbers
    contraCorrectOL = contraCorrectOL(randperm(newlengthOL));
    ipsiCorrectOL = ipsiCorrectOL(randperm(newlengthOL));
    contraErrorsOL = contraErrorsOL(randperm(newlengthOL));
    ipsiErrorsOL = ipsiErrorsOL(randperm(newlengthOL));
    
    contraCorrectOR = contraCorrectOR(randperm(newlengthOR));
    ipsiCorrectOR = ipsiCorrectOR(randperm(newlengthOR));
    contraErrorsOR = contraErrorsOR(randperm(newlengthOR));
    ipsiErrorsOR = contraErrorsOR(randperm(newlengthOR));
    ...
    %abort if not enough trials
    if newlengthOL < 10 || newlengthOR < 10
        disp('Too few trials, continuing...')
        return
    end
...
outdir = '/volumes/Dump/Analyses/Errors/subsample/PDF/';
if PDFflag == 1
...
outdir = '/volumes/Dump/Analyses/Errors/subsample/';
