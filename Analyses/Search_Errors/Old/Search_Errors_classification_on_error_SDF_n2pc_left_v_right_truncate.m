...
%     end
    ...
    %Check to see if truncated P2Pc timing is within 5 ms of non-truncated
    %TDT for CORRECT TRIALS
    %truncate AD channels 20 ms before saccade
    
    EEG_trunc = truncateAD_targ(EEG,SRT);
    TDT_correct_trunc = getTDT_AD(EEG_trunc,contra_correct_full,ipsi_correct_full);
    
    if abs(TDT_correct_trunc - nosubTDT.correct_EEG) > 5
    ...
    nosub_wf.EEG.error_contra = nanmean(EEG(inTrial_error_full,:));
    ...
    eval(['print -dpdf /volumes/Dump/Analyses/Errors/classification_on_error_SDF_check_truncTime/PDF/' file_name{sess} '_' DSPname{sess}])
    ...
    c = ',';
    qcq = [q c q];
    outdir = '/volumes/Dump/Analyses/Errors/classification_on_error_SDF_check_truncTime/Matrices/';
