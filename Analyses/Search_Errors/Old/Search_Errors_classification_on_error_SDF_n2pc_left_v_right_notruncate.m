...
for sess = 1:length(file_name)
    ...
    if Hemi.(EEGname{sess}) == 'R'
        EEG_RF = [3 4 5];
        EEG_antiRF = [7 0 1];
    elseif Hemi.(EEGname{sess}) == 'L'
        EEG_RF = [7 0 1];
        EEG_antiRF = [3 4 5];
    end
    
    contra_correct_full = shake(find(Correct_(:,2) == 1 & ismember(Target_(:,2),EEG_RF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsi_correct_full = shake(find(Correct_(:,2) == 1 & ismember(Target_(:,2),EEG_antiRF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    
    contra_error_full = shake(find(Errors_(:,5) == 1 & ismember(Target_(:,2),EEG_RF) & ismember(saccLoc,EEG_antiRF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
    ipsi_error_full = shake(find(Errors_(:,5) == 1 & ismember(Target_(:,2),EEG_antiRF) & ismember(saccLoc,EEG_RF) & SRT(:,1) < 2000 & SRT(:,1) > 50));
...
%     end
    ...
    nosubTDT.correct_EEG = getTDT_AD(EEG,contra_correct_full,ipsi_correct_full);
    ...
    nosubTDT.error_EEG = getTDT_AD(EEG,contra_error_full,ipsi_error_full);
    ...
    nosub_wf.EEG.correct_contra = nanmean(EEG(contra_correct_full,:));
    nosub_wf.EEG.correct_ipsi = nanmean(EEG(ipsi_correct_full,:));
    nosub_wf.EEG.error_contra = nanmean(EEG(contra_error_full,:));
    nosub_wf.EEG.error_ipsi = nanmean(EEG(ipsi_error_full,:));
    ...
    newLength_contra = findMin(length(contra_correct_full),length(contra_error_full));
    newLength_ipsi = findMin(length(ipsi_correct_full),length(ipsi_error_full));
    ...
    for rep = 1:nReps
        ...
        contra_correct = shake(contra_correct_full);
        ipsi_correct = shake(ipsi_correct_full);
        contra_error = shake(contra_error_full);
        ipsi_error = shake(ipsi_error_full);
...
%         outTrials_error = outTrials_error(randperm(newLength_out));
        contra_correct = contra_correct(randperm(newLength_contra));
        ipsi_correct = ipsi_correct(randperm(newLength_ipsi));
%         contra_error = contra_error(randperm(newLength_contra));
%         ipsi_error = ipsi_error(randperm(newLength_ipsi));
        ...
        subTDT.correct_EEG(rep,1) = getTDT_AD(EEG,contra_correct,ipsi_correct);
        ...
        sub_wf.EEG.correct_contra(rep,1:3001) = nanmean(EEG(contra_correct,:));
        sub_wf.EEG.correct_ipsi(rep,1:3001) = nanmean(EEG(ipsi_correct,:));
    ...
    sub_n.correct_contra = newLength_contra;
    sub_n.correct_ipsi = newLength_ipsi;
    ...
    eval(['print -dpdf /volumes/Dump/Analyses/Errors/classification_on_error_SDF_subsample/truncate/PDF/' file_name{sess} '_' DSPname{sess}])
    ...
    c = ',';
    qcq = [q c q];
    outdir = '/volumes/Dump/Analyses/Errors/classification_on_error_SDF_subsample/truncate/Matrices/';
