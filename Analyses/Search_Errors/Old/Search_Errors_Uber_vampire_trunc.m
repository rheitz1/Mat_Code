...
function [] = Search_Errors_Uber_vampire_trunc(file)
...
% PDFdir = '~/desktop/temp_trunc/PDF/';
% MATdir = '~/desktop/temp_trunc/Matrices/';

PDFdir = '//scratch/heitzrp/Output/Search_Errors_trunc/PDF/';
MATdir = '//scratch/heitzrp/Output/Search_Errors_trunc/Matrices/';
...
if isempty(DSPlist)
    ...
    for DSPchan = 1:size(DSPlist,1)
        ...
        if isempty(RF)
            ...
            %NOTE: I am not requiring that trial n-1 NOT be a catch trial.
            %Few sessions actually have enough catch trials to make a
            %difference, and for those sessions, the additional criteria
            %removes too many trials.  As well, it is unclear whether, say,
            %an error following a direction error is different from an
            %error following a catch error.
