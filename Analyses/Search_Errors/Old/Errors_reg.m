...
for file = 1:size(file_name,1)
    ...
    inTrials_correct = find(ismember(Target_(:,2),RF) & Correct_(:,2) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50);
    outTrials_correct = find(ismember(Target_(:,2),anti_RF) & Correct_(:,2) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 50);

    %error when target in RF but saccade into anti_RF
    inTrials_incorrect = find(ismember(Target_(:,2),RF) & ismember(SaccDir_,anti_RF) & SRT(:,1) < 2000 & SRT(:,1) > 50);
    %error when target in antiRF but saccade into RF
    outTrials_incorrect = find(ismember(Target_(:,2),anti_RF) & ismember(SaccDir_,RF) & SRT(:,1) < 2000 & SRT(:,1) > 50);
    ...
    [TDT_correct_ROC cROCarea] = ROC(Spikes,Plot_Time_s,Align_Time_s,inTrials_correct,outTrials_correct);
    %flip inTrials and outTrials for error responses because they tend
    %to be in the other direction.  ROC function will run normally
    [TDT_incorrect_ROC eROCarea] = ROC(Spikes,Plot_Time_s,Align_Time_s,outTrials_incorrect,inTrials_incorrect);
    ...
    for time = 1:size(SDFs_in_correct,2)
        [p_correct(time),h_correct(time)] = ranksum(SDFs_in_correct(:,time),SDFs_out_correct(:,time),'alpha',.05);
        [p_incorrect(time),h_incorrect(time)] = ranksum(SDFs_in_incorrect(:,time),SDFs_out_incorrect(:,time),'alpha',.05);
    end
    ...
    if ~isempty(sigRuns_correct)
        TDT_correct_Wilcoxon = sigRuns_correct(1) - 50 + 100;
    else
        TDT_correct_Wilcoxon = NaN;
    end

    if ~isempty(sigRuns_incorrect)
        TDT_incorrect_Wilcoxon = sigRuns_incorrect(1) - 50 + 100;
    else
        TDT_incorrect_Wilcoxon = NaN;
    end
    cTDT_ROC(file,1) = TDT_correct_ROC;
    eTDT_ROC(file,1) = TDT_incorrect_ROC;
    cTDT_Wilcoxon(file,1) = TDT_correct_Wilcoxon;
    eTDT_Wilcoxon(file,1) = TDT_incorrect_Wilcoxon;
    cRT(file,1) = nanmean(SRT(find(Correct_(:,2) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 100)),1);
    %RTs for direction errors
    eRT(file,1) = nanmean(SRT(find(Errors_(:,5) == 1 & SRT(:,1) < 2000 & SRT(:,1) > 100)),1);
    %also keep track of file names
    filename(file,1) = file_name(file);
    ...
    if plotFlag == 1
        ...
        if ~isnan(TDT_correct_Wilcoxon)
            line([TDT_correct_Wilcoxon TDT_correct_Wilcoxon],[min(min(SDFs_in_correct_s),min(SDFs_out_correct_s)) SDFs_in_correct_s(TDT_correct_Wilcoxon + abs(Plot_Time_s(1)))],'Color','b')
        end
        if ~isnan(TDT_correct_ROC)
            line([TDT_correct_ROC TDT_correct_ROC],[min(min(SDFs_in_correct_s),min(SDFs_out_correct_s)) SDFs_in_correct_s(TDT_correct_ROC + abs(Plot_Time_s(1)))],'Color','b','LineStyle','--')
        end
        if ~isnan(TDT_incorrect_Wilcoxon)
            line([TDT_incorrect_Wilcoxon TDT_incorrect_Wilcoxon],[min(min(SDFs_in_incorrect_s),min(SDFs_out_incorrect_s)) SDFs_in_incorrect_s(TDT_incorrect_Wilcoxon + abs(Plot_Time_s(1)))],'Color','r')
        end
        if ~isnan(TDT_incorrect_ROC)
            line([TDT_incorrect_ROC TDT_incorrect_ROC],[min(min(SDFs_in_incorrect_s),min(SDFs_out_incorrect_s)) SDFs_in_incorrect_s(TDT_incorrect_ROC + abs(Plot_Time_s(1)))],'Color','r','LineStyle','--')
        end
        ...
        title(['T in Correct: ' mat2str(length(inTrials_correct)) ' T out Correct: ' mat2str(length(outTrials_correct)) ' T in Incorrect: ' mat2str(length(inTrials_incorrect)) ' T out Incorrect: ' mat2str(length(outTrials_incorrect))],'fontsize',12)
        ...
        if pdfFlag == 1
            eval(['print -dpdf /volumes/Dump/Analyses/Errors/',cell2mat(file_name(file)),'_',cell2mat(cell_name(file)),'-VisMove_reg.pdf'])
            %             eval(['print -dpdf ',outdir,batch_list(i).name,'_',cell2mat(CellNames(k)),'_1s.pdf']);
        end
    end
    keep cRT eRT cTDT_ROC cTDT_Wilcoxon eTDT_ROC eTDT_Wilcoxon plotFlag pdfFlag file file_name cell_name f_path q c qcq
    close all
end