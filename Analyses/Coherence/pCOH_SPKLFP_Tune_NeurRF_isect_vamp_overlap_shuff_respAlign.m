...
function [] = pCOH_SPKLFP_Tune_NeurRF_isect_vamp_overlap_shuff_respAlign(file_name,sig1_name,sig2_name)
...
%if LFP and SPIKE from the same electrode, do the interpolation.  Check to see if they are from the same
%electrode
% id1 = str2num(sig1_name(3:4));
% id2 = str2num(sig2_name(4:5));
% 
% %Remove influence of spikes on the LFP.  Use a -2 to +2 window
% if id1 == id2
%     disp('Interpolating LFP')
...
sig1 = response_align(sig1,SRT,[-400 200]);
sig1_bc = baseline_correct(sig1,[1 100]);
...
%Response-aligned SDFs
wf.sig2.in.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.all,TrialStart_);
wf.sig2.in.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.ss2,TrialStart_);
wf.sig2.in.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.ss4,TrialStart_);
wf.sig2.in.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.ss8,TrialStart_);
wf.sig2.in.fast.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.fast.ss2,TrialStart_);
wf.sig2.in.fast.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.fast.ss4,TrialStart_);
wf.sig2.in.fast.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.fast.ss8,TrialStart_);
wf.sig2.in.slow.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.slow.ss2,TrialStart_);
wf.sig2.in.slow.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.slow.ss4,TrialStart_);
wf.sig2.in.slow.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.slow.ss8,TrialStart_);
wf.sig2.in.fast.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.fast.all,TrialStart_);
wf.sig2.in.slow.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.slow.all,TrialStart_);
wf.sig2.in.err = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],in.err,TrialStart_);
 
wf.sig2.out.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.all,TrialStart_);
wf.sig2.out.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.ss2,TrialStart_);
wf.sig2.out.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.ss4,TrialStart_);
wf.sig2.out.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.ss8,TrialStart_);
wf.sig2.out.fast.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.fast.ss2,TrialStart_);
wf.sig2.out.fast.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.fast.ss4,TrialStart_);
wf.sig2.out.fast.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.fast.ss8,TrialStart_);
wf.sig2.out.slow.ss2 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.slow.ss2,TrialStart_);
wf.sig2.out.slow.ss4 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.slow.ss4,TrialStart_);
wf.sig2.out.slow.ss8 = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.slow.ss8,TrialStart_);
wf.sig2.out.fast.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.fast.all,TrialStart_);
wf.sig2.out.slow.all = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.slow.all,TrialStart_);
wf.sig2.out.err = spikedensityfunct(sig2,SRT(:,1)+500,[-400 200],out.err,TrialStart_);
...
%Now we will subtract the SRT+500 from each spike time
subt = repmat(SRT(:,1)+500,1,size(sig2,2));
sig2 = sig2 - subt;
...
if shuffFlag == 1
    ...
    [~,f_shuff,tout_shuff,~,~,~,~,~,~,~,~,between_cond_shuff.all,~,~] = Spk_LFPCoh(sig2(alltr,:),sig1(alltr,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    
    %with shuffling (pad = 0) & vsBaseline test
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.all] = Spk_LFPCoh(sig2(in.all,:),sig1(in.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.all] = Spk_LFPCoh(sig2(out.all,:),sig1(out.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2,ClusShuffOpts);
end
 
 
% ALL TRIALS
try
    [coh.in.all,f,tout,Sx.in.all,Sy.in.all,Pcoh.in.all,PSx.in.all,PSy.in.all,] = Spk_LFPCoh(sig2(in.all,:),sig1(in.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.all,f,tout,Sx.out.all,Sy.out.all,Pcoh.out.all,PSx.out.all,PSy.out.all] = Spk_LFPCoh(sig2(out.all,:),sig1(out.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
catch
...
% SS2
try
    [coh.in.ss2,f,tout,Sx.in.ss2,Sy.in.ss2,Pcoh.in.ss2,PSx.in.ss2,PSy.in.ss2] = Spk_LFPCoh(sig2(in.ss2,:),sig1(in.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.ss2,f,tout,Sx.out.ss2,Sy.out.ss2,Pcoh.out.ss2,PSx.out.ss2,PSy.out.ss2] = Spk_LFPCoh(sig2(out.ss2,:),sig1(out.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('SS2')
catch
...
% SS4
try
    [coh.in.ss4,f,tout,Sx.in.ss4,Sy.in.ss4,Pcoh.in.ss4,PSx.in.ss4,PSy.in.ss4] = Spk_LFPCoh(sig2(in.ss4,:),sig1(in.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.ss4,f,tout,Sx.out.ss4,Sy.out.ss4,Pcoh.out.ss4,PSx.out.ss4,PSy.out.ss4] = Spk_LFPCoh(sig2(out.ss4,:),sig1(out.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('SS4')
catch
...
% SS8
try
    [coh.in.ss8,f,tout,Sx.in.ss8,Sy.in.ss8,Pcoh.in.ss8,PSx.in.ss8,PSy.in.ss8] = Spk_LFPCoh(sig2(in.ss8,:),sig1(in.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.ss8,f,tout,Sx.out.ss8,Sy.out.ss8,Pcoh.out.ss8,PSx.out.ss8,PSy.out.ss8] = Spk_LFPCoh(sig2(out.ss8,:),sig1(out.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('SS8')
catch
...
% FAST, SS2
try
    [coh.in.fast.ss2,f,tout,Sx.in.fast.ss2,Sy.in.fast.ss2,Pcoh.in.fast.ss2,PSx.in.fast.ss2,PSy.in.fast.ss2] = Spk_LFPCoh(sig2(in.fast.ss2,:),sig1(in.fast.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.fast.ss2,f,tout,Sx.out.fast.ss2,Sy.out.fast.ss2,Pcoh.out.fast.ss2,PSx.out.fast.ss2,PSy.out.fast.ss2] = Spk_LFPCoh(sig2(out.fast.ss2,:),sig1(out.fast.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Fast, SS2')
catch
...
% FAST, SS4
try
    [coh.in.fast.ss4,f,tout,Sx.in.fast.ss4,Sy.in.fast.ss4,Pcoh.in.fast.ss4,PSx.in.fast.ss4,PSy.in.fast.ss4] = Spk_LFPCoh(sig2(in.fast.ss4,:),sig1(in.fast.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.fast.ss4,f,tout,Sx.out.fast.ss4,Sy.out.fast.ss4,Pcoh.out.fast.ss4,PSx.out.fast.ss4,PSy.out.fast.ss4] = Spk_LFPCoh(sig2(out.fast.ss4,:),sig1(out.fast.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Fast, SS4')
catch
...
% FAST, SS8
try
    [coh.in.fast.ss8,f,tout,Sx.in.fast.ss8,Sy.in.fast.ss8,Pcoh.in.fast.ss8,PSx.in.fast.ss8,PSy.in.fast.ss8] = Spk_LFPCoh(sig2(in.fast.ss8,:),sig1(in.fast.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.fast.ss8,f,tout,Sx.out.fast.ss8,Sy.out.fast.ss8,Pcoh.out.fast.ss8,PSx.out.fast.ss8,PSy.out.fast.ss8] = Spk_LFPCoh(sig2(out.fast.ss8,:),sig1(out.fast.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Fast, SS8')
catch
...
% SLOW, SS2
try
    [coh.in.slow.ss2,f,tout,Sx.in.slow.ss2,Sy.in.slow.ss2,Pcoh.in.slow.ss2,PSx.in.slow.ss2,PSy.in.slow.ss2] = Spk_LFPCoh(sig2(in.slow.ss2,:),sig1(in.slow.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.slow.ss2,f,tout,Sx.out.slow.ss2,Sy.out.slow.ss2,Pcoh.out.slow.ss2,PSx.out.slow.ss2,PSy.out.slow.ss2] = Spk_LFPCoh(sig2(out.slow.ss2,:),sig1(out.slow.ss2,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Slow, SS2')
catch
...
% SLOW SS4
try
    [coh.in.slow.ss4,f,tout,Sx.in.slow.ss4,Sy.in.slow.ss4,Pcoh.in.slow.ss4,PSx.in.slow.ss4,PSy.in.slow.ss4] = Spk_LFPCoh(sig2(in.slow.ss4,:),sig1(in.slow.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.slow.ss4,f,tout,Sx.out.slow.ss4,Sy.out.slow.ss4,Pcoh.out.slow.ss4,PSx.out.slow.ss4,PSy.out.slow.ss4] = Spk_LFPCoh(sig2(out.slow.ss4,:),sig1(out.slow.ss4,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Slow, SS4')
catch
...
% SLOW, SS8
try
    [coh.in.slow.ss8,f,tout,Sx.in.slow.ss8,Sy.in.slow.ss8,Pcoh.in.slow.ss8,PSx.in.slow.ss8,PSy.in.slow.ss8] = Spk_LFPCoh(sig2(in.slow.ss8,:),sig1(in.slow.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.slow.ss8,f,tout,Sx.out.slow.ss8,Sy.out.slow.ss8,Pcoh.out.slow.ss8,PSx.out.slow.ss8,PSy.out.slow.ss8] = Spk_LFPCoh(sig2(out.slow.ss8,:),sig1(out.slow.ss8,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Slow, SS8')
catch
...
% FAST, ALL SET SIZES
try
    [coh.in.fast.all,f,tout,Sx.in.fast.all,Sy.in.fast.all,Pcoh.in.fast.all,PSx.in.fast.all,PSy.in.fast.all] = Spk_LFPCoh(sig2(in.fast.all,:),sig1(in.fast.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.fast.all,f,tout,Sx.out.fast.all,Sy.out.fast.all,Pcoh.out.fast.all,PSx.out.fast.all,PSy.out.fast.all] = Spk_LFPCoh(sig2(out.fast.all,:),sig1(out.fast.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Fast, all SS')
catch
...
% SLOW, ALL SET SIZES
try
    [coh.in.slow.all,f,tout,Sx.in.slow.all,Sy.in.slow.all,Pcoh.in.slow.all,PSx.in.slow.all,PSy.in.slow.all] = Spk_LFPCoh(sig2(in.slow.all,:),sig1(in.slow.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    [coh.out.slow.all,f,tout,Sx.out.slow.all,Sy.out.slow.all,Pcoh.out.slow.all,PSx.out.slow.all,PSy.out.slow.all] = Spk_LFPCoh(sig2(out.slow.all,:),sig1(out.slow.all,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
    disp('Slow, all SS')
catch
...
if errskip == 0
    try
        [coh.in.err,f,tout,Sx.in.err,Sy.in.err,Pcoh.in.err,PSx.in.err,PSy.in.err] = Spk_LFPCoh(sig2(in.err,:),sig1(in.err,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
        [coh.out.err,f,tout,Sx.out.err,Sy.out.err,Pcoh.out.err,PSx.out.err,PSy.out.err] = Spk_LFPCoh(sig2(out.err,:),sig1(out.err,:),tapers,-400,[-400 200], 1000, .01, [0 100],0,4,.05,0,2,2);
        disp('Errors')
    catch
...
if errskip == 0
    for rep = 1:nReps
        ...
        [tempcoh.in.all_sub, f, tout, tempSx.in.all_sub, tempSy.in.all_sub, tempPcoh.in.all_sub, tempPSx.in.all_sub, tempPSy.in.all_sub] = Spk_LFPCoh(sig2(in.all_sub,:),sig1(in.all_sub,:),tapers,-400,[-400 200],1000,.01,[0 100],0,4,.05,0,2,2);
        [tempcoh.out.all_sub, f, tout, tempSx.out.all_sub, tempSy.out.all_sub, tempPcoh.out.all_sub, tempPSx.out.all_sub, tempPSy.out.all_sub] = Spk_LFPCoh(sig2(out.all_sub,:),sig1(out.all_sub,:),tapers,-400,[-400 200],1000,.01,[0 100],0,4,.05,0,2,2);
