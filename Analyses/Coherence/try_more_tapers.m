...
function [] = try_more_tapers(file_name,sig1_name,sig2_name)
...
ClusShuffOpts.nShuffs = 2; %number of shuffles for between condition tests

%ClusShuffOpts.BasePer = [-500 100];
%ClusShuffOpts.ActPer = [-200 600];
...
%tapers = PreGenTapers([.6 5]);
tapers = PreGenTapers([.2 15]);
...
if saveFlag == 1
    save(['~/desktop/' file_name '_' ...
        sig1_name sig2_name '.mat'],'within_cond_shuff','coh','f','n','tout','Sx','Sy', ...
