...
function [] = pSpec_SPKLFP_Tune_NeurRF_isect_vamp_nooverlap_interp(file_name,sig1_name,sig2_name)
...
% ALL TRIALS
%try
    [Spec.LFP.in.all,f,tout,pSpec.LFP.in.all] = LFPSpec(sig1(in.all,:),tapers,1000, .01, [0 200],0, -500, 2, 4, .05, 0,2,0);
    [Spec.LFP.out.all,f,tout,pSpec.LFP.out.all] = LFPSpec(sig1(out.all,:),tapers,1000, .01, [0 200],0, -500, 2, 4, .05, 0,2,0);
    
    [Spec.SPK.in.all,f,tout,pSpec.SPK.in.all] = SpkSpec(sig2(in.all,:),tapers,[-500 2500],1000,.01, [0 200],0, 4, .05, 0,2,0,[],2);
    [Spec.SPK.out.all,f,tout,pSpec.SPK.out.all] = SpkSpec(sig2(out.all,:),tapers,[-500 2500],1000,.01, [0 200],0, 4, .05, 0,2,0,[],2);
% catch
...
if saveFlag == 1
    save(['~/desktop/nooverlap/' file_name '_' ...
        sig1_name sig2_name '.mat'],'Spec','pSpec','wf','tout','f','-mat')
end
