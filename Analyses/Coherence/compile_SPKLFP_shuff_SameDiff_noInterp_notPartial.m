...
for sess = 1:length(file_list);
    ...
    coh_all.in.all_sub(1:281,1:104,sess) = coh.in.all_sub(:,find(f < 101));
    coh_all.in.all(1:281,1:104,sess) = coh.in.all(:,find(f < 101));
    coh_all.in.ss2(1:281,1:104,sess) = coh.in.ss2(:,find(f < 101));
    coh_all.in.ss4(1:281,1:104,sess) = coh.in.ss4(:,find(f < 101));
    coh_all.in.ss8(1:281,1:104,sess) = coh.in.ss8(:,find(f < 101));
    coh_all.in.fast.ss2(1:281,1:104,sess) = coh.in.fast.ss2(:,find(f < 101));
    coh_all.in.fast.ss4(1:281,1:104,sess) = coh.in.fast.ss4(:,find(f < 101));
    coh_all.in.fast.ss8(1:281,1:104,sess) = coh.in.fast.ss8(:,find(f < 101));
    coh_all.in.slow.ss2(1:281,1:104,sess) = coh.in.slow.ss2(:,find(f < 101));
    coh_all.in.slow.ss4(1:281,1:104,sess) = coh.in.slow.ss4(:,find(f < 101));
    coh_all.in.slow.ss8(1:281,1:104,sess) = coh.in.slow.ss8(:,find(f < 101));
    coh_all.in.fast.all(1:281,1:104,sess) = coh.in.fast.all(:,find(f < 101));
    coh_all.in.slow.all(1:281,1:104,sess) = coh.in.slow.all(:,find(f < 101));
    coh_all.in.err(1:281,1:104,sess) = coh.in.err(:,find(f < 101));
    
    
    coh_all.out.all_sub(1:281,1:104,sess) = coh.out.all_sub(:,find(f < 101));
    coh_all.out.all(1:281,1:104,sess) = coh.out.all(:,find(f < 101));
    coh_all.out.ss2(1:281,1:104,sess) = coh.out.ss2(:,find(f < 101));
    coh_all.out.ss4(1:281,1:104,sess) = coh.out.ss4(:,find(f < 101));
    coh_all.out.ss8(1:281,1:104,sess) = coh.out.ss8(:,find(f < 101));
    coh_all.out.fast.ss2(1:281,1:104,sess) = coh.out.fast.ss2(:,find(f < 101));
    coh_all.out.fast.ss4(1:281,1:104,sess) = coh.out.fast.ss4(:,find(f < 101));
    coh_all.out.fast.ss8(1:281,1:104,sess) = coh.out.fast.ss8(:,find(f < 101));
    coh_all.out.slow.ss2(1:281,1:104,sess) = coh.out.slow.ss2(:,find(f < 101));
    coh_all.out.slow.ss4(1:281,1:104,sess) = coh.out.slow.ss4(:,find(f < 101));
    coh_all.out.slow.ss8(1:281,1:104,sess) = coh.out.slow.ss8(:,find(f < 101));
    coh_all.out.fast.all(1:281,1:104,sess) = coh.out.fast.all(:,find(f < 101));
    coh_all.out.slow.all(1:281,1:104,sess) = coh.out.slow.all(:,find(f < 101));
    coh_all.out.err(1:281,1:104,sess) = coh.out.err(:,find(f < 101));
    
    coh_all_101_200.in.all_sub(1:281,1:102,sess) = coh.in.all_sub(:,find(f >= 101));
    coh_all_101_200.in.all(1:281,1:102,sess) = coh.in.all(:,find(f >= 101));
    coh_all_101_200.in.ss2(1:281,1:102,sess) = coh.in.ss2(:,find(f >= 101));
    coh_all_101_200.in.ss4(1:281,1:102,sess) = coh.in.ss4(:,find(f >= 101));
    coh_all_101_200.in.ss8(1:281,1:102,sess) = coh.in.ss8(:,find(f >= 101));
    coh_all_101_200.in.fast.ss2(1:281,1:102,sess) = coh.in.fast.ss2(:,find(f >= 101));
    coh_all_101_200.in.fast.ss4(1:281,1:102,sess) = coh.in.fast.ss4(:,find(f >= 101));
    coh_all_101_200.in.fast.ss8(1:281,1:102,sess) = coh.in.fast.ss8(:,find(f >= 101));
    coh_all_101_200.in.fast.all(1:281,1:102,sess) = coh.in.fast.all(:,find(f >= 101));
    coh_all_101_200.in.slow.ss2(1:281,1:102,sess) = coh.in.slow.ss2(:,find(f >= 101));
    coh_all_101_200.in.slow.ss4(1:281,1:102,sess) = coh.in.slow.ss4(:,find(f >= 101));
    coh_all_101_200.in.slow.ss8(1:281,1:102,sess) = coh.in.slow.ss8(:,find(f >= 101));
    coh_all_101_200.in.slow.all(1:281,1:102,sess) = coh.in.slow.all(:,find(f >= 101));
    coh_all_101_200.in.err(1:281,1:102,sess) = coh.in.err(:,find(f >= 101));
    
    
    coh_all_101_200.out.all_sub(1:281,1:102,sess) = coh.out.all_sub(:,find(f >= 101));
    coh_all_101_200.out.all(1:281,1:102,sess) = coh.out.all(:,find(f >= 101));
    coh_all_101_200.out.ss2(1:281,1:102,sess) = coh.out.ss2(:,find(f >= 101));
    coh_all_101_200.out.ss4(1:281,1:102,sess) = coh.out.ss4(:,find(f >= 101));
    coh_all_101_200.out.ss8(1:281,1:102,sess) = coh.out.ss8(:,find(f >= 101));
    coh_all_101_200.out.fast.ss2(1:281,1:102,sess) = coh.out.fast.ss2(:,find(f >= 101));
    coh_all_101_200.out.fast.ss4(1:281,1:102,sess) = coh.out.fast.ss4(:,find(f >= 101));
    coh_all_101_200.out.fast.ss8(1:281,1:102,sess) = coh.out.fast.ss8(:,find(f >= 101));
    coh_all_101_200.out.slow.ss2(1:281,1:102,sess) = coh.out.slow.ss2(:,find(f >= 101));
    coh_all_101_200.out.slow.ss4(1:281,1:102,sess) = coh.out.slow.ss4(:,find(f >= 101));
    coh_all_101_200.out.slow.ss8(1:281,1:102,sess) = coh.out.slow.ss8(:,find(f >= 101));
    coh_all_101_200.out.fast.all(1:281,1:102,sess) = coh.out.fast.all(:,find(f >= 101));
    coh_all_101_200.out.slow.all(1:281,1:102,sess) = coh.out.slow.all(:,find(f >= 101));
    coh_all_101_200.out.err(1:281,1:102,sess) = coh.out.err(:,find(f >= 101));
    
    
    
    %========================
    shuff_all.in.all.coh(1:281,1:41,sess) = between_cond_shuff.all.TrType1.coh;
    shuff_all.out.all.coh(1:281,1:41,sess) = between_cond_shuff.all.TrType2.coh;
    ...
    keep tout tout_shuff f f_shuff f_top coh_all coh_all_101_200 shuff_all n TDT_all wf_all file_list sess plotFlag
