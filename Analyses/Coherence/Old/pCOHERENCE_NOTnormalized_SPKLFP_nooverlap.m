...
[file_name sig1_name sig2_name] = textread('SPKLFP_nooverlap.txt', '%s %s %s');
...
for sess = 1:length(file_name)
    ...
    %for no overlap condition, use neuron RF
    ...
    if saveFlag == 1
        save(['/volumes/Dump2/Coherence/Uber/SPK-LFP_nooverlap_notnormalized/' file_name{sess} '_' ...
