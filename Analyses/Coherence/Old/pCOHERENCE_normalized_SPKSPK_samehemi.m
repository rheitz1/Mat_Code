...
%inspection). THESE COMPARISONS ARE ONLY FOR NEURONS WITH OVERLAPPING RFS
%THAT WERE ALSO RECORDED IN THE SAME HEMISPHERE ON DIFFERENT PHYSICAL
%ELECTRODES.


[file_name sig1_name sig2_name] = textread('SPKSPK_samehemi.txt', '%s %s %s');
...
for sess = 1:length(file_name)
    ...
    [a, b, c, Sx_noz.out.all, Sy_noz.out.all, d, PSx_noz.out.all, PSy_noz.out.all] = Spk_SpkCoh(sig1(in.all,:),sig2(in.all,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.all, f, tout, Sx.out.all, Sy.out.all, Pcoh.out.all, PSx.out.all, PSy.out.all] = Spk_SpkCoh(sig1(in.all,:),sig2(in.all,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.ss2, Sy_noz.out.ss2, d, PSx_noz.out.ss2, PSy_noz.out.ss2] = Spk_SpkCoh(sig1(in.ss2,:),sig2(in.ss2,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.ss2, f, tout, Sx.out.ss2, Sy.out.ss2, Pcoh.out.ss2, PSx.out.ss2, PSy.out.ss2] = Spk_SpkCoh(sig1(in.ss2,:),sig2(in.ss2,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.ss4, Sy_noz.out.ss4, d, PSx_noz.out.ss4, PSy_noz.out.ss4] = Spk_SpkCoh(sig1(in.ss4,:),sig2(in.ss4,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.ss4, f, tout, Sx.out.ss4, Sy.out.ss4, Pcoh.out.ss4, PSx.out.ss4, PSy.out.ss4] = Spk_SpkCoh(sig1(in.ss4,:),sig2(in.ss4,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.ss8, Sy_noz.out.ss8, d, PSx_noz.out.ss8, PSy_noz.out.ss8] = Spk_SpkCoh(sig1(in.ss8,:),sig2(in.ss8,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.ss8, f, tout, Sx.out.ss8, Sy.out.ss8, Pcoh.out.ss8, PSx.out.ss8, PSy.out.ss8] = Spk_SpkCoh(sig1(in.ss8,:),sig2(in.ss8,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.fast, Sy_noz.out.fast, d, PSx_noz.out.fast, PSy_noz.out.fast] = Spk_SpkCoh(sig1(in.fast,:),sig2(in.fast,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.fast, f, tout, Sx.out.fast, Sy.out.fast, Pcoh.out.fast, PSx.out.fast, PSy.out.fast] = Spk_SpkCoh(sig1(in.fast,:),sig2(in.fast,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.slow, Sy_noz.out.slow, d, PSx_noz.out.slow, PSy_noz.out.slow] = Spk_SpkCoh(sig1(in.slow,:),sig2(in.slow,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.slow, f, tout, Sx.out.slow, Sy.out.slow, Pcoh.out.slow, PSx.out.slow, PSy.out.slow] = Spk_SpkCoh(sig1(in.slow,:),sig2(in.slow,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    
    [a, b, c, Sx_noz.out.err, Sy_noz.out.err, d, PSx_noz.out.err, PSy_noz.out.err] = Spk_SpkCoh(sig1(in.err,:),sig2(in.err,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
    [coh.out.err, f, tout, Sx.out.err, Sy.out.err, Pcoh.out.err, PSx.out.err, PSy.out.err] = Spk_SpkCoh(sig1(in.err,:),sig2(in.err,:),tapers,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,1);
    ...
    if saveFlag == 1
        save(['/volumes/Dump2/Coherence/Uber/SPK-LFPXXXX/' file_name{sess} '_' ...
