...
function [] = pCOHERENCE_normalized_SPKLFP_Hemi_NeurRF_isect_vamp_nooverlap(file_name,sig1_name,sig2_name)
...
%Can not overlap; just use neuron RF

%double-check to make sure no overlap
if Hemi.(sig1_name) == 'R'
    RF1 = [3 4 5];
elseif Hemi.(sig1_name) == 'L'
    RF1 = [7 0 1];
else
    error('LFP RF Problem')
end

RF2 = RFs.(sig2_name);

if ~isempty(intersect(RF1,RF2))
    disp('Overlap of RFs detected; Skipping...')
    return
end


%Now that we know there is no overlap, set to neuron's RF.
RF = RFs.(sig2_name);
antiRF = mod((RF+4),8);
...
if errskip == 0
    ...
    Sx_noz.in.err(1:length(tout),1:length(f)) = NaN;
    Sy_noz.in.err(1:length(tout),1:length(f)) = NaN;
    PSx_noz.in.err(1:length(tout),1:length(f)) = NaN;
    PSy_noz.in.err(1:length(tout),1:length(f)) = NaN;
...
if errskip == 0
    ...
    Sx_noz.out.err(1:length(tout),1:length(f)) = NaN;
    Sy_noz.out.err(1:length(tout),1:length(f)) = NaN;
    PSx_noz.out.err(1:length(tout),1:length(f)) = NaN;
    PSy_noz.out.err(1:length(tout),1:length(f)) = NaN;
...
if errskip == 0
    for rep = 1:nReps
        ...
        [a, b, c, tempSx_noz.in.all_sub, tempSy_noz.in.all_sub, d, tempPSx_noz.in.all_sub, tempPSy_noz.in.all_sub] = Spk_LFPCoh(sig2(in.all_sub,:),sig1(in.all_sub,:),tapers,-500,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
        ...
        [a, b, c, tempSx_noz.out.all_sub, tempSy_noz.out.all_sub, d, tempPSx_noz.out.all_sub, tempPSy_noz.out.all_sub] = Spk_LFPCoh(sig2(out.all_sub,:),sig1(out.all_sub,:),tapers,-500,[-500 2500],1000,.01,[0 100],0,4,.05,0,2,0);
        ...
        Sx_noz.in.all_sub(1:length(tout),1:length(f),rep) = tempSx_noz.in.all_sub;
        Sy_noz.in.all_sub(1:length(tout),1:length(f),rep) = tempSy_noz.in.all_sub;
        PSx_noz.in.all_sub(1:length(tout),1:length(f),rep) = tempPSx_noz.in.all_sub;
        PSy_noz.in.all_sub(1:length(tout),1:length(f),rep) = tempPSy_noz.in.all_sub;
        ...
        Sx_noz.out.all_sub(1:length(tout),1:length(f),rep) = tempSx_noz.out.all_sub;
        Sy_noz.out.all_sub(1:length(tout),1:length(f),rep) = tempSy_noz.out.all_sub;
        PSx_noz.out.all_sub(1:length(tout),1:length(f),rep) = tempPSx_noz.out.all_sub;
        PSy_noz.out.all_sub(1:length(tout),1:length(f),rep) = tempPSy_noz.out.all_sub;
    ...
    Sx_noz.in.all_sub = mean(Sx_noz.in.all_sub,3);
    Sy_noz.in.all_sub = mean(Sy_noz.in.all_sub,3);
    PSx_noz.in.all_sub = mean(PSx_noz.in.all_sub,3);
    PSy_noz.in.all_sub = mean(PSy_noz.in.all_sub,3);
    ...
    Sx_noz.out.all_sub = mean(Sx_noz.out.all_sub,3);
    Sy_noz.out.all_sub = mean(Sy_noz.out.all_sub,3);
    PSx_noz.out.all_sub = mean(PSx_noz.out.all_sub,3);
    PSy_noz.out.all_sub = mean(PSy_noz.out.all_sub,3);
    ...
    Sx_noz.in.all_sub(1:length(tout),1:length(f)) = NaN;
    Sy_noz.in.all_sub(1:length(tout),1:length(f)) = NaN;
    PSx_noz.in.all_sub(1:length(tout),1:length(f)) = NaN;
    PSy_noz.in.all_sub(1:length(tout),1:length(f)) = NaN;
    ...
    Sx_noz.out.all_sub(1:length(tout),1:length(f)) = NaN;
    Sy_noz.out.all_sub(1:length(tout),1:length(f)) = NaN;
    PSx_noz.out.all_sub(1:length(tout),1:length(f)) = NaN;
    PSy_noz.out.all_sub(1:length(tout),1:length(f)) = NaN;
...
if saveFlag == 1
    save(['//scratch/heitzrp/Output/Coherence/Uber_Hemi_NeuronRF_intersection/nooverlap/' file_name '_' ...
        sig1_name sig2_name '.mat'],'coh','f','tout','Sx','Sy', ...
        'RT','Pcoh','PSx','PSy','wf','TDT','Sx_noz','Sy_noz', ...
        'PSx_noz','PSy_noz','-mat')
end
