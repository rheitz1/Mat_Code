...
%
%LFP-EEG comparisons:  Set EEG RF to contralateral hemifield.  Use overlap
%of LFP RF (from tuning function LFPtuning) and the EEG hemifield RF, but
%we want to make sure that the LFP has a contralateral RF as well.  This
%version does not truncate the EEG 20 ms before the saccade but that is
%simply implemented

% THIS VERSION WORKS FOR BOTH REGULAR TL AND uSTIM TL

function [] = pCOHERENCE_LFPEEG_Tune_Hemi_isect_vamp_overlap_shuff(file_name,LFP_name)
...
load(file_name,LFP_name, ...
    'AD02','AD03','MStim_','Hemi','EyeX_','EyeY_','Target_','Errors_','newfile','Correct_','SRT','TrialStart_')
...
%use selectivity of LFP to determine its RF. Also make sure there is a
%usable TDT <= 350 ms
[RF1 ~ ~ ~ ~ ~ TDT] = LFPtuning(LFP);

if isnan(TDT)
    disp('No TDT...moving on')
    return
elseif TDT > 350
    disp('TDT > 350 ms...moving on')
    return
end

if Hemi.(LFP_name) == 'L' & ~isempty(ismember(RF1,[7 0 1]))
    contra_EEG = AD03;
    ipsi_EEG = AD02;
    cond = 'LFP contraRF';
elseif Hemi.(LFP_name) == 'L' & ~isempty(ismember(RF1,[3 4 5]))
    contra_EEG = AD02;
    ipsi_EEG = AD03;
    cond = 'LFP ipsiRF';
elseif Hemi.(LFP_name) == 'R' & ~isempty(ismember(RF1,[7 0 1]))
    contra_EEG = AD03;
    ipsi_EEG = AD02;
    cond = 'LFP ipsiRF';
elseif Hemi.(LFP_name) == 'R' & ~isempty(ismember(RF1,[3 4 5]))
    contra_EEG = AD02;
    ipsiEEG = AD03;
    cond = 'LFP contraRF';
else
    disp('No Matching LFP RF')
end
...
%now invert signals so phase plots are referenced to *negativity*
sig1 = -sig1;
sig2 = -sig2;
...
%find trrial vectors; deal with uStim data
if exist('MStim_') == 0
...
elseif exist('MStim_') == 1
    in.fast = find(isnan(MStim_(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),RF) & SRT(:,1) < cMed & SRT(:,1) > 50);
    in.slow = find(isnan(MStim_(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),RF) & SRT(:,1) >= cMed & SRT(:,1) < 2000);
    in.err = find(isnan(MStim_(:,1)) & Errors_(:,5) == 1 & ismember(Target_(:,2),RF) & ismember(saccLoc,antiRF) & SRT(:,1) < 2000 & SRT(:,1) > 50);
    
    out.fast = find(isnan(MStim_(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),antiRF) & SRT(:,1) < cMed & SRT(:,1) > 50);
    out.slow = find(isnan(MStim_(:,1)) & Correct_(:,2) == 1 & ismember(Target_(:,2),antiRF) & SRT(:,1) >= cMed & SRT(:,1) < 2000);
...
if shuffFlag == 1
    ...
    [~,f_shuff,~,~,~,~,~,~,~,~,~,between_cond_shuff.all,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ...
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.all] = LFP_LFPCoh(sig1(in.all,:),sig2(in.all,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.all] = LFP_LFPCoh(sig1(out.all,:),sig2(out.all,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.all,f,tout,Sx.in.all,Sy.in.all,Pcoh.in.all,PSx.in.all,PSy.in.all,] = LFP_LFPCoh(sig1(in.all,:),sig2(in.all,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.all,f,tout,Sx.out.all,Sy.out.all,Pcoh.out.all,PSx.out.all,PSy.out.all] = LFP_LFPCoh(sig1(out.all,:),sig2(out.all,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
%=======================
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.ss2,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    ...
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.ss2] = LFP_LFPCoh(sig1(in.ss2,:),sig2(in.ss2,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.ss2] = LFP_LFPCoh(sig1(out.ss2,:),sig2(out.ss2,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.ss2,f,tout,Sx.in.ss2,Sy.in.ss2,Pcoh.in.ss2,PSx.in.ss2,PSy.in.ss2,] = LFP_LFPCoh(sig1(in.ss2,:),sig2(in.ss2,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.ss2,f,tout,Sx.out.ss2,Sy.out.ss2,Pcoh.out.ss2,PSx.out.ss2,PSy.out.ss2] = LFP_LFPCoh(sig1(out.ss2,:),sig2(out.ss2,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
%=======================
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.ss4,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    ...
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.ss4] = LFP_LFPCoh(sig1(in.ss4,:),sig2(in.ss4,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.ss4] = LFP_LFPCoh(sig1(out.ss4,:),sig2(out.ss4,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.ss4,f,tout,Sx.in.ss4,Sy.in.ss4,Pcoh.in.ss4,PSx.in.ss4,PSy.in.ss4,] = LFP_LFPCoh(sig1(in.ss4,:),sig2(in.ss4,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.ss4,f,tout,Sx.out.ss4,Sy.out.ss4,Pcoh.out.ss4,PSx.out.ss4,PSy.out.ss4] = LFP_LFPCoh(sig1(out.ss4,:),sig2(out.ss4,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.ss8,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    ...
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.ss8] = LFP_LFPCoh(sig1(in.ss8,:),sig2(in.ss8,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.ss8] = LFP_LFPCoh(sig1(out.ss8,:),sig2(out.ss8,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.ss8,f,tout,Sx.in.ss8,Sy.in.ss8,Pcoh.in.ss8,PSx.in.ss8,PSy.in.ss8,] = LFP_LFPCoh(sig1(in.ss8,:),sig2(in.ss8,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.ss8,f,tout,Sx.out.ss8,Sy.out.ss8,Pcoh.out.ss8,PSx.out.ss8,PSy.out.ss8] = LFP_LFPCoh(sig1(out.ss8,:),sig2(out.ss8,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.fast,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    ...
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.fast] = LFP_LFPCoh(sig1(in.fast,:),sig2(in.fast,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.fast] = LFP_LFPCoh(sig1(out.fast,:),sig2(out.fast,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.fast,f,tout,Sx.in.fast,Sy.in.fast,Pcoh.in.fast,PSx.in.fast,PSy.in.fast,] = LFP_LFPCoh(sig1(in.fast,:),sig2(in.fast,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.fast,f,tout,Sx.out.fast,Sy.out.fast,Pcoh.out.fast,PSx.out.fast,PSy.out.fast] = LFP_LFPCoh(sig1(out.fast,:),sig2(out.fast,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.slow,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
    ...
    ClusShuffOpts.TestType = 1;
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.slow] = LFP_LFPCoh(sig1(in.slow,:),sig2(in.slow,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.slow] = LFP_LFPCoh(sig1(out.slow,:),sig2(out.slow,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
end

%without shuffling (pad == 4) & no tests
[coh.in.slow,f,tout,Sx.in.slow,Sy.in.slow,Pcoh.in.slow,PSx.in.slow,PSy.in.slow,] = LFP_LFPCoh(sig1(in.slow,:),sig2(in.slow,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
[coh.out.slow,f,tout,Sx.out.slow,Sy.out.slow,Pcoh.out.slow,PSx.out.slow,PSy.out.slow] = LFP_LFPCoh(sig1(out.slow,:),sig2(out.slow,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
...
if shuffFlag == 1
    ...
    %with shuffling (pad = 0) & between condition tests
    [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.fast_slow,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    clear alltr
    ClusShuffOpts.TrType = [];
end
...
if errskip == 0
    ...
    if shuffFlag == 1
        ...
        %with shuffling (pad = 0) & between condition tests
        [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.err,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
        clear alltr
        ClusShuffOpts.TrType = [];
        
        %with shuffling (pad = 0) & vsBaseline test
        ClusShuffOpts.TestType = 1;
        [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.in.err] = LFP_LFPCoh(sig1(in.err,:),sig2(in.err,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
        [~,~,~,~,~,~,~,~,~,~,~,within_cond_shuff.out.err] = LFP_LFPCoh(sig1(out.err,:),sig2(out.err,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
    end
    
    %with shuffling (pad == 4) & no tests
    [coh.in.err,f,tout,Sx.in.err,Sy.in.err,Pcoh.in.err,PSx.in.err,PSy.in.err] = LFP_LFPCoh(sig1(in.err,:),sig2(in.err,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
    [coh.out.err,f,tout,Sx.out.err,Sy.out.err,Pcoh.out.err,PSx.out.err,PSy.out.err] = LFP_LFPCoh(sig1(out.err,:),sig2(out.err,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
    %=======================
    ...
    if shuffFlag == 1
        ...
        %with shuffling (pad = 0)  & between condition tests
        [~,~,~,~,~,~,~,~,~,~,~,between_cond_shuff.corr_err,~,~] = LFP_LFPCoh(sig1(alltr,:),sig2(alltr,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2,ClusShuffOpts);
        clear alltr
        ClusShuffOpts.TrType = [];
    end
...
if errskip == 0
    for rep = 1:nReps
        ...
        [tempcoh.in.all_sub, f, tout, tempSx.in.all_sub, tempSy.in.all_sub, tempPcoh.in.all_sub, tempPSx.in.all_sub, tempPSy.in.all_sub] = LFP_LFPCoh(sig1(in.all_sub,:),sig2(in.all_sub,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
        [tempcoh.out.all_sub, f, tout, tempSx.out.all_sub, tempSy.out.all_sub, tempPcoh.out.all_sub, tempPSx.out.all_sub, tempPSy.out.all_sub] = LFP_LFPCoh(sig1(out.all_sub,:),sig2(out.all_sub,:),tapers,1000,.01,[0 100],0,-500,0,4,.05,0,2,2);
