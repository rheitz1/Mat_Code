...
function [] = COHERENCE_vampire_reg_UBER_shift_predictor(file)
tic
...
for pair = size(pairings,1):-1:1
    ...
    %if in same hemisphere & are EEG or LFP, compare contra vs. ipsi
    ...
    if strmatch('AD',chanlist(pairings(pair,1))) == 1 & strmatch('AD',chanlist(pairings(pair,2))) == 1
        ...
        if cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == cell2mat(Hemi(strmatch(chanlist(pairings(pair,2)),all_chan),2));
            %check to see which hemisphere they both are in
            if cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == 'R'
                chan1_hemi = {'R'};
                chan2_hemi = {'R'};
            ...
            elseif cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == 'L'
                chan1_hemi = {'L'};
                chan2_hemi = {'L'};
            ...
            if cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == 'L'
                chan1_hemi = {'L'};
            elseif cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == 'R'
                chan1_hemi = {'R'};
            elseif cell2mat(Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2)) == 'M'
                chan1_hemi = {'M'};
            end
            
            if cell2mat(Hemi(strmatch(chanlist(pairings(pair,2)),all_chan),2)) == 'L'
                chan2_hemi = {'L'};
            elseif cell2mat(Hemi(strmatch(chanlist(pairings(pair,2)),all_chan),2)) == 'R'
                chan2_hemi = {'R'};
            elseif cell2mat(Hemi(strmatch(chanlist(pairings(pair,2)),all_chan),2)) == 'M'
                chan2_hemi = {'M'};
            end
    ...
    elseif strmatch('AD',chanlist(pairings(pair,1))) == 1 & strmatch('DSP',chanlist(pairings(pair,2))) == 1
        ...
        chan1_hemi = Hemi(strmatch(chanlist(pairings(pair,1)),all_chan),2);
        
        %to find DSP channel hemi, convert name to ADxx and look up in
        %'Hemi' variable
        tempname = mat2str(cell2mat(chanlist(pairings(pair,2))));
        stub = tempname(end-3:end-2);
        newname = ['AD' stub];
        chan2_hemi = Hemi(strmatch(newname,all_chan),2);
        
        %check for RF.  If empty RF, do not compute comparison (cell bad)
        all_neurons_in_workspace = varlist(strmatch('DSP',varlist));
        RF = RFs{strmatch(chanlist(pairings(pair,2)),all_neurons_in_workspace)};
    ...
    elseif strmatch('DSP',chanlist(pairings(pair,1))) == 1 & strmatch('DSP',chanlist(pairings(pair,2))) == 1
        ...
        %to find DSP channel hemi, convert name to ADxx and look up in
        %'Hemi' variable
        tempname = mat2str(cell2mat(chanlist(pairings(pair,1))));
        stub = tempname(end-3:end-2);
        newname = ['AD' stub];
        chan1_hemi = Hemi(strmatch(newname,all_chan),2);
        ...
        %check for RF.  If empty RF, do not compute comparison (cell bad)
        all_neurons_in_workspace = varlist(strmatch('DSP',varlist));
        RF_1 = RFs{strmatch(chanlist(pairings(pair,1)),all_neurons_in_workspace)};
        RF_2 = RFs{strmatch(chanlist(pairings(pair,2)),all_neurons_in_workspace)};
    ...
    %=====================================
    % GET SHIFT PREDICTOR:  SHIFT SIGNAL 1 and 2
        sig1_targ = sig1_targ(randperm(size(sig1_targ,1)),:);
        sig1_resp = sig1_resp(randperm(size(sig1_resp,1)),:);
    
        sig2_targ = sig2_targ(randperm(size(sig2_targ,1)),:);
        sig2_resp = sig2_resp(randperm(size(sig2_resp,1)),:);
    %======================================
    ...
    elseif strmatch('AD',chanlist(pairings(pair,1))) == 1 & strmatch('DSP',chanlist(pairings(pair,2))) == 1
        [coh_targ.correct_nosub,f_targ,tout_targ,spec2_targ.correct_nosub,spec2_targ.correct_nosub] = Spk_LFPCoh(sig2_correct_targ_nosub,sig1_correct_targ_nosub,tapers,-500,[-500 2500],1000,.01,[0,200],0);
    ...
    if plotFlag == 1
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_nosub_v_sub.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_correct_v_errors.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_v_spectra_correct.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_correct_fast_v_slow.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_errors_fast_v_slow.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_SetSize.jpg',q])
        ...
        eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_spectra_SetSize.jpg',q])
        ...
        if exist('correct_contra') == 1
            ...
            eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_correct_contra_v_ipsi.jpg',q])
        ...
        if exist('correct_Tin') == 1
            ...
            eval(['print -djpeg100 ',q,'//scratch/heitzrp/Output/Coherence/Uber_Shift/JPG/',file,'_',[name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_correct_Tin_v_Din.jpg',q])
    ...
    if saveFlag == 1
        ...
        save(['//scratch/heitzrp/Output/Coherence/Uber_Shift/Matrices/' file '_' ...
            name1 name2 cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2))) ...
            '_Coherence_SHIFT_PREDICTOR.mat'],'coh_targ','coh_resp', 'n','RTs',...
            'spec1_targ','spec1_resp','spec2_targ', ...
    ...
    clear tout_targ tout_resp f_targ f_resp n color_limits ...
        ...
        name1 name2 sig* wf* coh* spec* RTs
