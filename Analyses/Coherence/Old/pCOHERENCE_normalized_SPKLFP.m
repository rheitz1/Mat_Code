...
for sess = 1:length(file_name)
 ...
 %   [SRT saccLoc] = getSRT(EyeX_,EyeY_);
    ...
    if Hemi.(sig1_name{sess}) == 'R'
        RF1 = [3 4 5];
    elseif Hemi.(sig1_name{sess}) == 'L'
        RF1 = [7 0 1];
    else
        error('RF Problem')
    end
    ...
    if isempty(RF)
        disp(['ERROR ' file_name{sess} ' ' sig1_name{sess} ' ' sig2_name{sess}])
    else
        ...
        disp(['In ' mat2str(length(in.all)) ' Out ' mat2str(length(out.all))])
        keep file_name sig1_name sig2_name sess
        continue
    ...
    if saveFlag == 1
        save(['/volumes/Dump2/Coherence/Uber/Normalized/SPK-LFP/' file_name{sess} '_' ...
