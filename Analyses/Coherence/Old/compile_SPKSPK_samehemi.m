%compile partial coherence poster analyses for SPK-SPK, ANY HEMISPHERE

%LFPLFP
cd /Volumes/Dump2/Coherence/Uber/SPK-SPK/
%cd ~/desktop/Poster/SPK-SPK/anyhemi/
...
if plotFlag == 1
    ...
    %power spectra (not z-transformed)
    figure
    subplot(3,2,1)
    ...
    subplot(3,2,2)
    imagesc(tout,f,nanmean(PSy_noz_all.in,3)')
    axis xy
    xlim([-50 500])
    colorbar
    title('Sig2 In')
    ...
    subplot(3,2,4)
    imagesc(tout,f,nanmean(PSy_noz_all.out,3)')
    axis xy
    xlim([-50 500])
    colorbar
    title('Sig2 Out')
    
    %no log scaling for spike spectra
    z1 = nanmean(PSx_noz_all.in,3)' - nanmean(PSx_noz_all.out,3)';
    z2 = nanmean(PSy_noz_all.in,3)' - nanmean(PSy_noz_all.out,3)';
