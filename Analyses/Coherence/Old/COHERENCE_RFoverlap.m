...
function [] = COHERENCE_RFoverlap(file)
...
%load DSP channels
[file_list neuron1 neuron2] = textread('RFoverlap.txt','%s %s %s');
...
for f = 1:size(file_list,1)
    ...
    correct_out = find(ismember(Target_(:,2),overlapAntiRF) & Correct_(:,2) == 1 & SRT(:,1) <= 2000 & SRT(:,1) > 50);

    %equate for number of trials since Coherence has a positive bias with # of
    %trials. Should really take a random draw but for now just take first n
    %observations
    minTrials = min(length(correct_in),length(correct_out));
    correct_in = correct_in(1:minTrials);
    correct_out = correct_out(1:minTrials);

    %abort if no trials
    if isempty(correct_in) || isempty(correct_out)
        disp('No Trials')
    ...
    sig1_correct_targ_out = sig1_targ(correct_out,:);

    % Sig 2, target-aligned
    sig2_correct_targ_in = sig2_targ(correct_in,:);
    sig2_correct_targ_out = sig2_targ(correct_out,:);
    %========================================================
    ...
    wf_sig1_targ.correct_out = spikedensityfunct(sig1_targ, Align_Time, Plot_Time, correct_out, TrialStart_);

    wf_sig2_targ.correct_in = spikedensityfunct(sig2_targ, Align_Time, Plot_Time, correct_in, TrialStart_);
    wf_sig2_targ.correct_out = spikedensityfunct(sig2_targ, Align_Time, Plot_Time, correct_out, TrialStart_);
    ...
    n.correct_out = size(sig1_correct_targ_out,1);
    ...
    [coh_targ.correct_out,f_targ,tout_targ,spec1_targ.correct_out,spec2_targ.correct_out] = Spk_SpkCoh(sig1_correct_targ_out,sig2_correct_targ_out,tapers,[-500 2500],1000,.01,[0,200],0);
    ...
    subplot(2,2,2)
    imagesc(tout_targ,f_targ,log10(spec1_targ.correct_out'))
    axis xy
    xlabel('Time from Target','fontweight','bold','fontsize',14)
    ylabel('Frequency','fontweight','bold','fontsize',14)
    xlim([-200 800])
    colorbar
    title([cell2mat(neuron1(f)) ' Spectra Correct OutTrials'],'fontweight','bold','fontsize',14)
    ...
    subplot(2,2,4)
    imagesc(tout_targ,f_targ,log10(spec2_targ.correct_out'))
    axis xy
    xlabel('Time from Target','fontweight','bold','fontsize',14)
    ylabel('Frequency','fontweight','bold','fontsize',14)
    xlim([-200 800])
    ...
    [ax,h2] = suplabel(['nCorrect InTrials = ' mat2str(n.correct_in) ' nCorrect OutTrials = ' mat2str(n.correct_out)],'y');
    ...
    eval(['print -djpeg100 ',q,'/volumes/Dump2/Coherence/RFoverlap/JPG/',cell2mat(file_list(f)),'_',[cell2mat(neuron1(f)) cell2mat(neuron2(f))],'in_v_out_spectra.jpg',q])
    ...
    xlim([-200 800])
    colorbar
    title([cell2mat(neuron1(f)) ' Coherence Correct InTrials'],'fontweight','bold','fontsize',14)
    color_limits(1,1:2) = get(gca,'CLim');

    subplot(1,2,2)
    imagesc(tout_targ,f_targ,abs(coh_targ.correct_out'))
    axis xy
    xlabel('Time from Target','fontweight','bold','fontsize',14)
    ylabel('Frequency','fontweight','bold','fontsize',14)
    xlim([-200 800])
    colorbar
    title([cell2mat(neuron1(f)) ' Coherence Correct OutTrials'],'fontweight','bold','fontsize',14)
    color_limits(2,1:2) = get(gca,'CLim');
    ...
    eval(['print -djpeg100 ',q,'/volumes/Dump2/Coherence/RFoverlap/JPG/',cell2mat(file_list(f)),'_',[cell2mat(neuron1(f)) cell2mat(neuron2(f))],'in_v_out_coherence.jpg',q])
    %eval(['print -djpeg100 ',q,'~/desktop/s/',file,'_',[cell2mat(chanlist(pairings(pair,1))) cell2mat(chanlist(pairings(pair,2)))],'_coherence_fastslow_errors.jpg',q])
    close(h)
    ...
    if saveFlag == 1
        ...
        save(['/volumes/Dump2/Coherence/RFoverlap/Matrices/' cell2mat(file_list(f)) '_' ...
            cell2mat(neuron1(f)) cell2mat(neuron2(f)) ...
            '_Coherence.mat'],'wf_sig1_targ','wf_sig2_targ','n', ...
