...
function [] = SDF_SL_SAT(Spike,align,Plot_Time)
...
%Preallocate
SDF.slow(1:length(Plot_Time(1):Plot_Time(2)),1:8) = NaN;
SDF.fast = SDF.slow;
...
for j = 0:7
    ...
    [SDF.slow(1:length(Plot_Time(1):Plot_Time(2)),j+1)] = spikedensityfunct(Spike, Align_Time, Plot_Time, intersect(find(Target_(trials,2) == j),slow_correct_made_dead), TrialStart_);
    [SDF.fast(1:length(Plot_Time(1):Plot_Time(2)),j+1)] = spikedensityfunct(Spike, Align_Time, Plot_Time, intersect(find(Target_(trials,2) == j),fast_correct_made_dead_withCleared), TrialStart_);
...
maxFires(1) = max(max(SDF.slow));
maxFires(2) = max(max(SDF.fast));
maxFire = max(maxFires);
...
for j = 0:7
    ...
    plot(Plot_Time(1):Plot_Time(2),SDF.slow(:,j+1),'r',Plot_Time(1):Plot_Time(2),SDF.fast(:,j+1),'g','LineWidth',.5)
    numticks = round(maxFire/5);
    set(gca,'YTick',0:numticks:maxFire); %control number of tick marks for visual clarity
    
    if align == 'r'; vline(0,'k'); end
