...
% [filename9 unit9] = textread('SAT_Vis_NoMed_Q.txt','%s %s'); 
% [filename10 unit10] = textread('SAT_Vis_Med_Q.txt','%s %s');
% [filename11 unit11] = textread('SAT_Vis_NoMed_S.txt','%s %s');
% [filename12 unit12] = textread('SAT_Vis_Med_S.txt','%s %s');
...
dsk  %in this version make sure we are not mistakenly in the 'longBase' directory
...
for k = 1:length(leak)
    ...
    for file = 1:length(filename)
        ...
        readout(1:size(SDF_r,1),1:1201) = 0;
        for trl = 1:size(SDF_r,1)
            ...
            readout(trl,1000-SRT(trl,1):1200) = trl_part_SDF;
        end
...
%         integrated_SDF(remove,:) = NaN;
        ...
        readout_in.slow_correct_made_dead(file,1:1201,k) = nanmean(readout(in.slow_correct_made_dead,:));
        readout_in.med_correct(file,1:1201,k) = nanmean(readout(in.med_correct,:));
        readout_in.fast_correct_made_dead(file,1:1201,k) = nanmean(readout(in.fast_correct_made_dead_withCleared,:));
        
        integrated_in.slow_correct_made_dead(file,1:1201,k) = nanmean(integrated_SDF(in.slow_correct_made_dead,:));
        integrated_in.med_correct(file,1:1201,k) = nanmean(integrated_SDF(in.med_correct,:));
        integrated_in.fast_correct_made_dead(file,1:1201,k) = nanmean(integrated_SDF(in.fast_correct_made_dead_withCleared,:));
        
        integrated_in.slow_correct_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.slow_correct_made_dead_binFAST,:));
        integrated_in.slow_correct_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.slow_correct_made_dead_binMED,:));
        integrated_in.slow_correct_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.slow_correct_made_dead_binSLOW,:));
        
        integrated_in.med_correct_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.med_correct_binFAST,:));
        integrated_in.med_correct_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.med_correct_binMED,:));
        integrated_in.med_correct_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.med_correct_binSLOW,:));
        
        integrated_in.fast_correct_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.fast_correct_made_dead_withCleared_binFAST,:));
        integrated_in.fast_correct_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.fast_correct_made_dead_withCleared_binMED,:));
        integrated_in.fast_correct_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.fast_correct_made_dead_withCleared_binSLOW,:));
        
        
        integrated_out.slow_correct_made_dead(file,1:1201,k) = nanmean(integrated_SDF(out.slow_correct_made_dead,:));
        integrated_out.med_correct(file,1:1201,k) = nanmean(integrated_SDF(out.med_correct,:));
        integrated_out.fast_correct_made_dead(file,1:1201,k) = nanmean(integrated_SDF(out.fast_correct_made_dead_withCleared,:));
        
        integrated_out.slow_correct_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.slow_correct_made_dead_binFAST,:));
        integrated_out.slow_correct_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.slow_correct_made_dead_binMED,:));
        integrated_out.slow_correct_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.slow_correct_made_dead_binSLOW,:));
        
        integrated_out.med_correct_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.med_correct_binFAST,:));
        integrated_out.med_correct_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.med_correct_binMED,:));
        integrated_out.med_correct_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.med_correct_binSLOW,:));
        
        integrated_out.fast_correct_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.fast_correct_made_dead_withCleared_binFAST,:));
        integrated_out.fast_correct_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.fast_correct_made_dead_withCleared_binMED,:));
        integrated_out.fast_correct_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.fast_correct_made_dead_withCleared_binSLOW,:));
        
        integrated_diff.slow_correct_made_dead(file,1:1201,k) = integrated_in.slow_correct_made_dead(file,1:1201,k) - integrated_out.slow_correct_made_dead(file,1:1201,k);
        integrated_diff.med_correct(file,1:1201,k) = integrated_in.med_correct(file,1:1201,k) - integrated_out.med_correct(file,1:1201,k);
        integrated_diff.fast_correct_made_dead(file,1:1201,k) = integrated_in.fast_correct_made_dead(file,1:1201,k) - integrated_out.fast_correct_made_dead(file,1:1201,k);
        
        integrated_diff.slow_correct_made_dead_binFast(file,1:1201,k) = integrated_in.slow_correct_made_dead_binFast(file,1:1201,k) - integrated_out.slow_correct_made_dead_binFast(file,1:1201,k);
        integrated_diff.slow_correct_made_dead_binMed(file,1:1201,k) = integrated_in.slow_correct_made_dead_binMed(file,1:1201,k) - integrated_out.slow_correct_made_dead_binMed(file,1:1201,k);
        integrated_diff.slow_correct_made_dead_binSlow(file,1:1201,k) = integrated_in.slow_correct_made_dead_binSlow(file,1:1201,k) - integrated_out.slow_correct_made_dead_binSlow(file,1:1201,k);
        
        integrated_diff.med_correct_binFast(file,1:1201,k) = integrated_in.med_correct_binFast(file,1:1201,k) - integrated_out.med_correct_binFast(file,1:1201,k);
        integrated_diff.med_correct_binMed(file,1:1201,k) = integrated_in.med_correct_binMed(file,1:1201,k) - integrated_out.med_correct_binMed(file,1:1201,k);
        integrated_diff.med_correct_binSlow(file,1:1201,k) = integrated_in.med_correct_binSlow(file,1:1201,k) - integrated_out.med_correct_binSlow(file,1:1201,k);
        
        integrated_diff.fast_correct_made_dead_binFast(file,1:1201,k) = integrated_in.fast_correct_made_dead_binFast(file,1:1201,k) - integrated_out.fast_correct_made_dead_binFast(file,1:1201,k);
        integrated_diff.fast_correct_made_dead_binMed(file,1:1201,k) = integrated_in.fast_correct_made_dead_binMed(file,1:1201,k) - integrated_out.fast_correct_made_dead_binMed(file,1:1201,k);
        integrated_diff.fast_correct_made_dead_binSlow(file,1:1201,k) = integrated_in.fast_correct_made_dead_binSlow(file,1:1201,k) - integrated_out.fast_correct_made_dead_binSlow(file,1:1201,k);
        ...
        leaky_integrated_in.slow_correct_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_correct_made_dead,:));
        leaky_integrated_in.med_correct(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_correct,:));
        leaky_integrated_in.fast_correct_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_correct_made_dead_withCleared,:));
        
        leaky_integrated_in.slow_correct_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_correct_made_dead_binFAST,:));
        leaky_integrated_in.slow_correct_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_correct_made_dead_binMED,:));
        leaky_integrated_in.slow_correct_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_correct_made_dead_binSLOW,:));
        
        leaky_integrated_in.med_correct_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_correct_binFAST,:));
        leaky_integrated_in.med_correct_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_correct_binMED,:));
        leaky_integrated_in.med_correct_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_correct_binSLOW,:));
        
        leaky_integrated_in.fast_correct_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_correct_made_dead_withCleared_binFAST,:));
        leaky_integrated_in.fast_correct_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_correct_made_dead_withCleared_binMED,:));
        leaky_integrated_in.fast_correct_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_correct_made_dead_withCleared_binSLOW,:));
        
        
        leaky_integrated_out.slow_correct_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_correct_made_dead,:));
        leaky_integrated_out.med_correct(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_correct,:));
        leaky_integrated_out.fast_correct_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_correct_made_dead_withCleared,:));
        
        leaky_integrated_out.slow_correct_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_correct_made_dead_binFAST,:));
        leaky_integrated_out.slow_correct_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_correct_made_dead_binMED,:));
        leaky_integrated_out.slow_correct_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_correct_made_dead_binSLOW,:));
        
        leaky_integrated_out.med_correct_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_correct_binFAST,:));
        leaky_integrated_out.med_correct_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_correct_binMED,:));
        leaky_integrated_out.med_correct_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_correct_binSLOW,:));
        
        leaky_integrated_out.fast_correct_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_correct_made_dead_withCleared_binFAST,:));
        leaky_integrated_out.fast_correct_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_correct_made_dead_withCleared_binMED,:));
        leaky_integrated_out.fast_correct_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_correct_made_dead_withCleared_binSLOW,:));
        
        
        
        leaky_integrated_diff.slow_correct_made_dead(file,1:1201,k) = leaky_integrated_in.slow_correct_made_dead(file,1:1201,k) - leaky_integrated_out.slow_correct_made_dead(file,1:1201,k);
        leaky_integrated_diff.med_correct(file,1:1201,k) = leaky_integrated_in.med_correct(file,1:1201,k) - leaky_integrated_out.med_correct(file,1:1201,k);
        leaky_integrated_diff.fast_correct_made_dead(file,1:1201,k) = leaky_integrated_in.fast_correct_made_dead(file,1:1201,k) - leaky_integrated_out.fast_correct_made_dead(file,1:1201,k);
        
        leaky_integrated_diff.slow_correct_made_dead_binFast(file,1:1201,k) = leaky_integrated_in.slow_correct_made_dead_binFast(file,1:1201,k) - leaky_integrated_out.slow_correct_made_dead_binFast(file,1:1201,k);
        leaky_integrated_diff.slow_correct_made_dead_binMed(file,1:1201,k) = leaky_integrated_in.slow_correct_made_dead_binMed(file,1:1201,k) - leaky_integrated_out.slow_correct_made_dead_binMed(file,1:1201,k);
        leaky_integrated_diff.slow_correct_made_dead_binSlow(file,1:1201,k) = leaky_integrated_in.slow_correct_made_dead_binSlow(file,1:1201,k) - leaky_integrated_out.slow_correct_made_dead_binSlow(file,1:1201,k);
        
        leaky_integrated_diff.med_correct_binFast(file,1:1201,k) = leaky_integrated_in.med_correct_binFast(file,1:1201,k) - leaky_integrated_out.med_correct_binFast(file,1:1201,k);
        leaky_integrated_diff.med_correct_binMed(file,1:1201,k) = leaky_integrated_in.med_correct_binMed(file,1:1201,k) - leaky_integrated_out.med_correct_binMed(file,1:1201,k);
        leaky_integrated_diff.med_correct_binSlow(file,1:1201,k) = leaky_integrated_in.med_correct_binSlow(file,1:1201,k) - leaky_integrated_out.med_correct_binSlow(file,1:1201,k);
        
        leaky_integrated_diff.fast_correct_made_dead_binFast(file,1:1201,k) = leaky_integrated_in.fast_correct_made_dead_binFast(file,1:1201,k) - leaky_integrated_out.fast_correct_made_dead_binFast(file,1:1201,k);
        leaky_integrated_diff.fast_correct_made_dead_binMed(file,1:1201,k) = leaky_integrated_in.fast_correct_made_dead_binMed(file,1:1201,k) - leaky_integrated_out.fast_correct_made_dead_binMed(file,1:1201,k);
        leaky_integrated_diff.fast_correct_made_dead_binSlow(file,1:1201,k) = leaky_integrated_in.fast_correct_made_dead_binSlow(file,1:1201,k) - leaky_integrated_out.fast_correct_made_dead_binSlow(file,1:1201,k);
        
        
        
        
        integrated_in.slow_errors_made_dead(file,1:1201,k) = nanmean(integrated_SDF(in.slow_errors_made_dead,:));
        integrated_in.med_errors(file,1:1201,k) = nanmean(integrated_SDF(in.med_errors,:));
        integrated_in.fast_errors_made_dead(file,1:1201,k) = nanmean(integrated_SDF(in.fast_errors_made_dead_withCleared,:));
        
        integrated_in.slow_errors_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.slow_errors_made_dead_binFAST,:));
        integrated_in.slow_errors_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.slow_errors_made_dead_binMED,:));
        integrated_in.slow_errors_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.slow_errors_made_dead_binSLOW,:));
        
        integrated_in.med_errors_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.med_errors_binFAST,:));
        integrated_in.med_errors_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.med_errors_binMED,:));
        integrated_in.med_errors_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.med_errors_binSLOW,:));
        
        integrated_in.fast_errors_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(in.fast_errors_made_dead_withCleared_binFAST,:));
        integrated_in.fast_errors_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(in.fast_errors_made_dead_withCleared_binMED,:));
        integrated_in.fast_errors_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(in.fast_errors_made_dead_withCleared_binSLOW,:));
        
        
        integrated_out.slow_errors_made_dead(file,1:1201,k) = nanmean(integrated_SDF(out.slow_errors_made_dead,:));
        integrated_out.med_errors(file,1:1201,k) = nanmean(integrated_SDF(out.med_errors,:));
        integrated_out.fast_errors_made_dead(file,1:1201,k) = nanmean(integrated_SDF(out.fast_errors_made_dead_withCleared,:));
        
        integrated_out.slow_errors_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.slow_errors_made_dead_binFAST,:));
        integrated_out.slow_errors_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.slow_errors_made_dead_binMED,:));
        integrated_out.slow_errors_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.slow_errors_made_dead_binSLOW,:));
        
        integrated_out.med_errors_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.med_errors_binFAST,:));
        integrated_out.med_errors_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.med_errors_binMED,:));
        integrated_out.med_errors_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.med_errors_binSLOW,:));
        
        integrated_out.fast_errors_made_dead_binFast(file,1:1201,k) = nanmean(integrated_SDF(out.fast_errors_made_dead_withCleared_binFAST,:));
        integrated_out.fast_errors_made_dead_binMed(file,1:1201,k) = nanmean(integrated_SDF(out.fast_errors_made_dead_withCleared_binMED,:));
        integrated_out.fast_errors_made_dead_binSlow(file,1:1201,k) = nanmean(integrated_SDF(out.fast_errors_made_dead_withCleared_binSLOW,:));
        
        integrated_diff.slow_errors_made_dead(file,1:1201,k) = integrated_in.slow_errors_made_dead(file,1:1201,k) - integrated_out.slow_errors_made_dead(file,1:1201,k);
        integrated_diff.med_errors(file,1:1201,k) = integrated_in.med_errors(file,1:1201,k) - integrated_out.med_errors(file,1:1201,k);
        integrated_diff.fast_errors_made_dead(file,1:1201,k) = integrated_in.fast_errors_made_dead(file,1:1201,k) - integrated_out.fast_errors_made_dead(file,1:1201,k);
        
        integrated_diff.slow_errors_made_dead_binFast(file,1:1201,k) = integrated_in.slow_errors_made_dead_binFast(file,1:1201,k) - integrated_out.slow_errors_made_dead_binFast(file,1:1201,k);
        integrated_diff.slow_errors_made_dead_binMed(file,1:1201,k) = integrated_in.slow_errors_made_dead_binMed(file,1:1201,k) - integrated_out.slow_errors_made_dead_binMed(file,1:1201,k);
        integrated_diff.slow_errors_made_dead_binSlow(file,1:1201,k) = integrated_in.slow_errors_made_dead_binSlow(file,1:1201,k) - integrated_out.slow_errors_made_dead_binSlow(file,1:1201,k);
        
        integrated_diff.med_errors_binFast(file,1:1201,k) = integrated_in.med_errors_binFast(file,1:1201,k) - integrated_out.med_errors_binFast(file,1:1201,k);
        integrated_diff.med_errors_binMed(file,1:1201,k) = integrated_in.med_errors_binMed(file,1:1201,k) - integrated_out.med_errors_binMed(file,1:1201,k);
        integrated_diff.med_errors_binSlow(file,1:1201,k) = integrated_in.med_errors_binSlow(file,1:1201,k) - integrated_out.med_errors_binSlow(file,1:1201,k);
        
        integrated_diff.fast_errors_made_dead_binFast(file,1:1201,k) = integrated_in.fast_errors_made_dead_binFast(file,1:1201,k) - integrated_out.fast_errors_made_dead_binFast(file,1:1201,k);
        integrated_diff.fast_errors_made_dead_binMed(file,1:1201,k) = integrated_in.fast_errors_made_dead_binMed(file,1:1201,k) - integrated_out.fast_errors_made_dead_binMed(file,1:1201,k);
        integrated_diff.fast_errors_made_dead_binSlow(file,1:1201,k) = integrated_in.fast_errors_made_dead_binSlow(file,1:1201,k) - integrated_out.fast_errors_made_dead_binSlow(file,1:1201,k);
        ...
        leaky_integrated_in.slow_errors_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_errors_made_dead,:));
        leaky_integrated_in.med_errors(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_errors,:));
        leaky_integrated_in.fast_errors_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_errors_made_dead_withCleared,:));
        
        
        leaky_integrated_in.slow_errors_made_missed(file,1:1201,k) = nanmean(leaky_integrated_SDF([in.slow_errors_made_dead ; in.slow_errors_missed_dead],:));
        leaky_integrated_in.fast_errors_made_missed(file,1:1201,k) = nanmean(leaky_integrated_SDF([in.fast_errors_made_dead_withCleared ; in.fast_errors_missed_dead_withCleared],:));
        
        
        
        leaky_integrated_in.slow_errors_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_errors_made_dead_binFAST,:));
        leaky_integrated_in.slow_errors_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_errors_made_dead_binMED,:));
        leaky_integrated_in.slow_errors_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.slow_errors_made_dead_binSLOW,:));
        
        leaky_integrated_in.med_errors_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_errors_binFAST,:));
        leaky_integrated_in.med_errors_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_errors_binMED,:));
        leaky_integrated_in.med_errors_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.med_errors_binSLOW,:));
        
        leaky_integrated_in.fast_errors_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_errors_made_dead_withCleared_binFAST,:));
        leaky_integrated_in.fast_errors_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_errors_made_dead_withCleared_binMED,:));
        leaky_integrated_in.fast_errors_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(in.fast_errors_made_dead_withCleared_binSLOW,:));
        
        
        leaky_integrated_out.slow_errors_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_errors_made_dead,:));
        leaky_integrated_out.med_errors(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_errors,:));
        leaky_integrated_out.fast_errors_made_dead(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_errors_made_dead_withCleared,:));
        
        
        leaky_integrated_out.slow_errors_made_missed(file,1:1201,k) = nanmean(leaky_integrated_SDF([out.slow_errors_made_dead ; in.slow_errors_missed_dead],:));
        leaky_integrated_out.fast_errors_made_missed(file,1:1201,k) = nanmean(leaky_integrated_SDF([out.fast_errors_made_dead_withCleared ; in.fast_errors_missed_dead_withCleared],:));
        
        
        leaky_integrated_out.slow_errors_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_errors_made_dead_binFAST,:));
        leaky_integrated_out.slow_errors_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_errors_made_dead_binMED,:));
        leaky_integrated_out.slow_errors_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.slow_errors_made_dead_binSLOW,:));
        
        leaky_integrated_out.med_errors_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_errors_binFAST,:));
        leaky_integrated_out.med_errors_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_errors_binMED,:));
        leaky_integrated_out.med_errors_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.med_errors_binSLOW,:));
        
        leaky_integrated_out.fast_errors_made_dead_binFast(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_errors_made_dead_withCleared_binFAST,:));
        leaky_integrated_out.fast_errors_made_dead_binMed(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_errors_made_dead_withCleared_binMED,:));
        leaky_integrated_out.fast_errors_made_dead_binSlow(file,1:1201,k) = nanmean(leaky_integrated_SDF(out.fast_errors_made_dead_withCleared_binSLOW,:));
        
        
        
        leaky_integrated_diff.slow_errors_made_dead(file,1:1201,k) = leaky_integrated_in.slow_errors_made_dead(file,1:1201,k) - leaky_integrated_out.slow_errors_made_dead(file,1:1201,k);
        leaky_integrated_diff.med_errors(file,1:1201,k) = leaky_integrated_in.med_errors(file,1:1201,k) - leaky_integrated_out.med_errors(file,1:1201,k);
        leaky_integrated_diff.fast_errors_made_dead(file,1:1201,k) = leaky_integrated_in.fast_errors_made_dead(file,1:1201,k) - leaky_integrated_out.fast_errors_made_dead(file,1:1201,k);
        
        leaky_integrated_diff.slow_errors_made_dead_binFast(file,1:1201,k) = leaky_integrated_in.slow_errors_made_dead_binFast(file,1:1201,k) - leaky_integrated_out.slow_errors_made_dead_binFast(file,1:1201,k);
        leaky_integrated_diff.slow_errors_made_dead_binMed(file,1:1201,k) = leaky_integrated_in.slow_errors_made_dead_binMed(file,1:1201,k) - leaky_integrated_out.slow_errors_made_dead_binMed(file,1:1201,k);
        leaky_integrated_diff.slow_errors_made_dead_binSlow(file,1:1201,k) = leaky_integrated_in.slow_errors_made_dead_binSlow(file,1:1201,k) - leaky_integrated_out.slow_errors_made_dead_binSlow(file,1:1201,k);
        
        leaky_integrated_diff.med_errors_binFast(file,1:1201,k) = leaky_integrated_in.med_errors_binFast(file,1:1201,k) - leaky_integrated_out.med_errors_binFast(file,1:1201,k);
        leaky_integrated_diff.med_errors_binMed(file,1:1201,k) = leaky_integrated_in.med_errors_binMed(file,1:1201,k) - leaky_integrated_out.med_errors_binMed(file,1:1201,k);
        leaky_integrated_diff.med_errors_binSlow(file,1:1201,k) = leaky_integrated_in.med_errors_binSlow(file,1:1201,k) - leaky_integrated_out.med_errors_binSlow(file,1:1201,k);
        
        leaky_integrated_diff.fast_errors_made_dead_binFast(file,1:1201,k) = leaky_integrated_in.fast_errors_made_dead_binFast(file,1:1201,k) - leaky_integrated_out.fast_errors_made_dead_binFast(file,1:1201,k);
        leaky_integrated_diff.fast_errors_made_dead_binMed(file,1:1201,k) = leaky_integrated_in.fast_errors_made_dead_binMed(file,1:1201,k) - leaky_integrated_out.fast_errors_made_dead_binMed(file,1:1201,k);
        leaky_integrated_diff.fast_errors_made_dead_binSlow(file,1:1201,k) = leaky_integrated_in.fast_errors_made_dead_binSlow(file,1:1201,k) - leaky_integrated_out.fast_errors_made_dead_binSlow(file,1:1201,k);
        ...
        keep k leak leakage filename unit file integrated_in integrated_out integrated_diff leaky_integrated_in leaky_integrated_out leaky_integrated_diff readout*
