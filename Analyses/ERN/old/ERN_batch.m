...
for sess = 1:size(file_name,1)
    ...
    correct = find(SRT(:,1) < 2000 & SRT(:,1) > 100 & Target_(:,2) ~= 255 & Correct_(:,2) == 1);
    errors = find(SRT(:,1) < 2000 & SRT(:,1) > 100 & Target_(:,2) ~=255 & Errors_(:,5) == 1);
...
%         newmax = max(max_cor,max_err);
        ...
        if plotFlag == 1
            if length(chanlist) <= 4
                subplot(2,2,chan)
                plot(Plot_Time(1):Plot_Time(2),nanmean(sig_correct_resp_bc),'b',Plot_Time(1):Plot_Time(2),nanmean(sig_errors_resp_bc),'--b',Plot_Time(1):Plot_Time(2),diffwave,'r')
                axis ij
    ...
    %
    if printFlag == 1
        eval(['print -dpdf ',q,'/volumes/Dump/Analyses/ERN/SEARCH_PDF/',cell2mat(file_name(sess)),'_ERN.pdf',q])
        close(f)
    end
    

    keep ERN *count file_name i q c qcq printFlag plotFlag findOnset Plot_Time
end
