...
[file_name] = textread('stimLeft.txt', '%s');
...
for sess = 1:length(file_name)
    ...
    ChanStruct = loadChan(file_name{sess},'LFP');
    ...
    ChanStruct = loadChan(file_name{sess},'DSP');
    ...
    for currLFP = 1:length(LFPlist)
        ...
        %Stimulating in the Left so all LFPs are in the right hemisphere
        RF = [3 4 5];
        antiRF = [7 0 1];
...
%             vline(TDT.LFP.nostim(totalLFP,1),'b')
            ...
            eval(['print -dpdf ',q,'/volumes/Dump/Analyses/uStim/TDT_uStim/StimLeft/PDF/',file_name{sess},'_',LFPlist{currLFP},'.pdf',q])
            close(f)
        end
    end
    %=======================================
    ...
    for currDSP = 1:length(DSPlist)
        ...
        if plotFlag == 1
            ...
            eval(['print -dpdf ',q,'/volumes/Dump/Analyses/uStim/TDT_uStim/StimLeft/PDF/',file_name{sess},LFPlist{currLFP},'spikes.pdf',q])
            close(f)
        end
    ...
    if plotFlag == 1
        ...
        eval(['print -dpdf ',q,'/volumes/Dump/Analyses/uStim/TDT_uStim/StimLeft/PDF/',file_name{sess},'_','OL.pdf',q])
        close(f)
    end
    ...
    if plotFlag == 1
        ...
        eval(['print -dpdf ',q,'/volumes/Dump/Analyses/uStim/TDT_uStim/StimLeft/PDF/',file_name{sess},'_','OR.pdf',q])
        close(f)
    end
...
if saveFlag == 1
    save('/volumes/Dump/Analyses/uStim/TDT_uStim/StimLeft/StimLeft.mat', ...
        'TDT','RT','ROC','wf','-mat')
end