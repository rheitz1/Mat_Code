...
for sess = 1:length(batch_list);
    ...
    if monkey == 'Q'
        ...
        allACC.contrastim.stim(sess,1) = length(trls_contra_stim) / length(all_contra_stim);
        allACC.ipsistim.stim(sess,1) = length(trls_ipsi_stim) / length(all_ipsi_stim);
        
        allACC.contrastim.nostim(sess,1) = length(trls_contra_nostim) / length(all_contra_nostim);
        allACC.ipsistim.nostim(sess,1) = length(trls_ipsi_nostim) / length(all_ipsi_nostim);
        
        keep monkey allROC allROC_trunc allTDT allTDT_trunc allRT allACC allwf allwf_trunc batch_list hemi sess plotFlag
