...
cd /Users/richardheitz/Desktop/Mat_Code/Analyses/JPSTH/PopOut/
[filename unit1a unit1b] = textread('JPSTH_PopOut_FEF_Q.txt','%s %s %s');
...
for file = 1:length(filename)
    ...
    load(filename{file},unit{file,1},unit{file,2},'TrialStart_','Correct_','Target_','SAT_','Errors_','newfile')
    filename{file}
...
if plotFlag
    ...
    subplot(221)
    imagesc(plottime_prestim(1):plottime_prestim(2),plottime_prestim(1):plottime_prestim(2), ...
        nanmean(allJPSTH.baseline.slow.normalized_jpsth,3))
    title('Accurate')
    colorbar
    axis xy
    %dline
    ...
    subplot(224)
    plot(plottime_prestim(1):win_size:plottime_prestim(2)-1,nanmean(allJPSTH.baseline.fast.pstch,3),'g')
    hold on
