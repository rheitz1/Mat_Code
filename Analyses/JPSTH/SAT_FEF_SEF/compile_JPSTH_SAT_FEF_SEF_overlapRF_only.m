...
%         disp('No overlapping RF...skipping')
%         continue
%     end
    
    in = find(ismember(Target_(:,2),intersect(RF1,RF2)));
    trls_all = intersect(in,[slow_correct_made_missed ; med_correct ; fast_correct_made_missed_withCleared]);
    trls_slow = intersect(in,slow_correct_made_missed);
    trls_fast = intersect(in,fast_correct_made_missed_withCleared);
    ...
    coin_width = 10; % average over coin_width/2 on either side of main diagonal
    ...
    %baseline activity
    out.full.all = getJPSTH(sig1,sig2,[plottime_full(1) plottime_full(2)],bin_width,coin_width,trls_all,0);
    out.full.slow = getJPSTH(sig1,sig2,[plottime_full(1) plottime_full(2)],bin_width,coin_width,trls_slow,0);
    out.full.fast = getJPSTH(sig1,sig2,[plottime_full(1) plottime_full(2)],bin_width,coin_width,trls_fast,0);
    
    
    out.baseline.all = getJPSTH(sig1,sig2,[plottime_prestim(1) plottime_prestim(2)],bin_width,coin_width,trls_all,0);
    out.baseline.slow = getJPSTH(sig1,sig2,[plottime_prestim(1) plottime_prestim(2)],bin_width,coin_width,trls_slow,0);
    out.baseline.fast = getJPSTH(sig1,sig2,[plottime_prestim(1) plottime_prestim(2)],bin_width,coin_width,trls_fast,0);
    
    out.stim.all = getJPSTH(sig1,sig2,[plottime_stim(1) plottime_stim(2)],bin_width,coin_width,trls_all,0);
    out.stim.slow = getJPSTH(sig1,sig2,[plottime_stim(1) plottime_stim(2)],bin_width,coin_width,trls_slow,0);
    out.stim.fast = getJPSTH(sig1,sig2,[plottime_stim(1) plottime_stim(2)],bin_width,coin_width,trls_fast,0);
    
    out.sacc.all = getJPSTH(sig1,sig2,[plottime_sacc(1) plottime_sacc(2)],bin_width,coin_width,trls_all,0);
    out.sacc.slow = getJPSTH(sig1,sig2,[plottime_sacc(1) plottime_sacc(2)],bin_width,coin_width,trls_slow,0);
    out.sacc.fast = getJPSTH(sig1,sig2,[plottime_sacc(1) plottime_sacc(2)],bin_width,coin_width,trls_fast,0);
    ...
    %     end
    %
    keep  file filename unit plotFlag plot_each_session plottime* allJPSTH n_overlapRF* bin_width coin_width sameHemi bin_centers
end
...
if plotFlag
    ...
    xlabel('Bin Center')
    hline(0,'k')
    title(['Covariogram over ' mat2str(coin_width) ' ms'])
    box off
