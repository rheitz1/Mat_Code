...
function [F] = animate_eye_SAT(trls,SpikeName)
...
for trl = 1:length(trls)
    ...
    subplot(5,4,1:3)
    set(gca,'fontsize',20)
    plot(-500:2500,EyeX_(trls(trl),:),'b',-500:2500,EyeY_(trls(trl),:),'r','linewidth',2)
    xlim([-500 2500])
...
%         set(gca,'linewidth',5)
        ...
        %for FAST trials, clear the display if it was in fact cleared
         if SAT_(trls(trl),11) == 1 & time > 501 & time > SAT_(trls(trl),3)+500 & didClear == 0
             didClear = 1;
             for distractor = 1:7
