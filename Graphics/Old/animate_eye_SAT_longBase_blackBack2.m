...
if exist('Stimuli_') == 0 | isempty(find(~isnan(Stimuli_))) == 1
    ...
    targ = Stimuli_(Target_(:,2)+1);
    
    %create distractor items.  For ease we will assume all distractors are
    %the same identity, even though that's not the case.
    dist(1:length(targ),1) = NaN;
    dist(find(targ == 4)) = 5;
    dist(find(targ == 5)) = 4;
    dist(find(targ > 5 & targ < 20)) = 11;
    dist(find(targ > 20)) = 21;
end
...
for trl = 1:length(trls)
    ...
    for time = round(Target_(1,1)-(abs(TIME_WINDOW(1))):speed:Target_(1,1)+TIME_WINDOW(2))
        ...
        if time >= Target_(1,1) && stimOn == 0
            ...
            dPos = setdiff([0:7],tPos);
            
            %draw target
            if Target_(trls(trl),2) > 5
            ...
            elseif Stimuli_(Target_(trls(trl),2)+1) == 4
                plot(circX(1,:)+x.pos7,circY(1,:)+y.pos7,'color','r','linewidth',50)
            elseif Stimuli_(Target_(trls(trl),2)+1) == 5
                plot(circX(1,:)+x.pos7,circY(1,:)+y.pos7,'--','color','g')
            end
