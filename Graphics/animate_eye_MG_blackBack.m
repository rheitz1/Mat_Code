...
function [] = animate_eye_MG_blackBack(trls,SpikeName)
...
if record_movie
    V = VideoWriter('~/desktop/testMG.avi');
    open(V);
end
...
if nargin == 2 && ~isempty(RF)
    ...
    SDF.in = spikedensityfunct(Spike,Target_(:,1),[-100 1000],in,TrialStart_);
    SDF.out = spikedensityfunct(Spike,Target_(:,1),[-100 1000],out,TrialStart_);
    TDT = getTDT_SP(Spike,in,out);
    
    subplot(5,4,[4 8])
    plot(-100:1000,SDF.in,'k',-100:1000,SDF.out,'--k','linewidth',2)
    xlim([-100 1000])
    vline(TDT,'k')
    title(['RF = ' mat2str(RF)])
...
%RPH: HARD CODING BASED ON SEARCH TASK - MG IS HIGHLY VARIABLE!
boxwidth = .7047;
boxheight = .6281;
...
for trl = 1:length(trls)
    ...
    stimOn = 0; %was target stim drawn yet?
    goOn = 0;   %was go signal (empty fixation point) drawn yet?
    stimOff = 0;%was MG stim cleared yet?
    reAppear = 0;%did target stim reappear at SRT?
...
%     end
    ...
    r = rectangle('position',[-.25,-.25,.4,.4],'edgecolor','white','facecolor','white','linewidth',10);
...
%         
        ...
        if time >= Target_(1,1) && stimOn == 0
            ...
            switch targpos
                case 0
                    TC = filledCircle([x.pos0 y.pos0],.2,100,'white');
                case 1
                    TC = filledCircle([x.pos1 y.pos1],.2,100,'white');
                case 2
                    TC = filledCircle([x.pos2 y.pos2],.2,100,'white');
                case 3
                    TC = filledCircle([x.pos3 y.pos3],.2,100,'white');
                case 4
                    TC = filledCircle([x.pos4 y.pos4],.2,100,'white');
                case 5
                    TC = filledCircle([x.pos5 y.pos5],.2,100,'white');
                case 6
                    TC = filledCircle([x.pos6 y.pos6],.2,100,'white');
                case 7
                    TC = filledCircle([x.pos7 y.pos7],.2,100,'white');
            end
            
        end
        
        %Turn off MG target stimuli after 100 ms
        
        if time >= Target_(1,1) + 50 && stimOff == 0
            stimOff = 1;
            set(TC,'visible','off')
        end
        
        
        %turn off filled fixation point (GO SIGNAL) at hold time
        if time >= Target_(1,1) + Target_(trls(trl),10) && goOn == 0
            goOn = 1;
            set(r,'facecolor','none')
        end
        
        %make target stim re-appear at SRT
        if time >= Target_(1,1) + SRT(trls(trl),1) && reAppear == 0
            reAppear = 1;
            set(TC,'visible','on')
        end
