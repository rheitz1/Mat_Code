...
function [] = animate_coherence(trls,SpikeName,ADName)

LFP = evalin('caller',ADName);
%LFP = filtSig(LFP,[50 80],'bandpass');
...
EyeX_ = baseline_correct(EyeX_,[400 500]);
EyeY_ = baseline_correct(EyeY_,[400 400]);
...
%     subplot(5,4,1:3)
%     plot(-500:2500,EyeX_(trls(trl),:),'b',-500:2500,EyeY_(trls(trl),:),'r')
%     xlim([-500 2500])
...
%     title(['Trial = ' mat2str(trls(trl))])
    ...
    if nargin >= 2 && ~isempty(RF)
        ...
        if ~isempty(Spike(trl,find(Spike(trl,:) <= 600)))
            contSpike = mySpk2Cont(Spike(trl,find(Spike(trl,:) <= 600)));
            ...
            %running it on
            spikeSound = audioplayer(contSpike,750);
            play(spikeSound)
        end
    end
    
    cur_vec(1:1101) = 0;
    for time = 1:20:1101%size(EyeX_,2)
        
        subplot(5,4,1:3)
        cur_vec(1:time) = LFP(trls(trl),1:time);
        plot(-500:600,cur_vec)
        xlim([-500 600])
        ylim([-.5 .5])
        %v = vline(time-500,'k');
