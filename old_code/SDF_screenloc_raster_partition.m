...
fast = ValidTrials(find(SRT(ValidTrials,1)) <= 400);
%mid = ValidTrials(find(SRT(ValidTrials,1) > 286 & SRT(ValidTrials,1)) <= 379);
slow = ValidTrials(find(SRT(ValidTrials,1)) > 900);
...
for k = 1:length(CellNames)
    ...
    for j = 0:7
        ...
        SDF=[];
        [SDF] = spikedensityfunct_lgn_old(CellName, Align_Time, SDFPlot_Time, fast(find(Target_(fast,2) == j)), TrialStart_);
        ...
        maxFire = 0;
        for SDF_j = 0:7
            Trls = slow(find(Target_(slow,2) == SDF_j));
