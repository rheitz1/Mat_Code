function[] = Translate_getWaves(file_path,filename)
...
%CUT UP AD CHANNEL
%ADTranslator_withCorrection(file_path,filename,TrialStart_,Strobe_Times,outfile)

%Cut Up Spikes
SpikeTranslator_getWaves(file_path,filename,TrialStart_,Strobe_Times,outfile)

%Get Spike Waveforms
%getWaves(file_path, fileID, outfile)

%Break into separate files for DET, MG, and SEARCH trials.
%breakFiles_waves(file_path, fileID, outfile)
