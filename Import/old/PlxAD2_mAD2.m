function PlxAD2_mAD2 (file_path,filename, TrialStart_,TimeStamps_Strobe_Events, outfile);
...
outfile2=[outfile(1:end-4) 'Long.mat'];
camspeed=240;
eyePosDelay=3*1000/camspeed;
...
%         delay = ceil(tsad(1)*1000);
        ...
        if strcmp('EyeX_',adID{ich+1})|strcmp('EyeY_',adID{ich+1})
            tsad=tsad-eyePosDelay/1000;
        end
        tsad(1:3)
        ...
        for curFrag = 1:length(tsad)
            ...
            %need to adjust for eyePosDelay
            if mod(curFrag,50000)==0
                disp(['On curFrag ' num2str(curFrag)])
            end
            if tsad(curFrag)<=0
                if tsad(curFrag+1)>0
                    disp(['adding part of Frag ' num2str(curFrag)]);
                    StartSample=2-ceil(tsad(curFrag)*1000); %if tsad(curFrag) is 0, you want to start at sample#2, if it's -1, you want to start at sample #3, etc.
                    Num2Add=fnad(curFrag)-(StartSample-1);
                    curTS=1:Num2Add;
                    eval( ['temp( curTS) = ',adID{ich+1},'( StartSample:StartSample+Num2Add-1);'])
                    ADindex=StartSample+Num2Add;
                else
                    disp(['skipped Frag ' num2str(curFrag)]);
                end
            else
...
%       AD01=temp;
        ...
        eval ([adID{ich+1} 'Long = subsamp(temp,4);']);
        clear temp
...
%         disp (['Delay between the start of recording and 1st AD timestamp...', num2str(length(delay)),'ms'])
        ...
        if lexist
            eval (['save  ([file_path,outfile2]',c,q, adID{ich+1},'Long',qcq,'-append',q, ');'])
        else
            eval (['save  ([file_path,outfile2]',c,q, adID{ich+1},'Long',q ');'])
            lexist=1;
        end
        disp ([adID{ich+1},'long....SAVED TO....',[file_path ,  outfile2] ]) 
