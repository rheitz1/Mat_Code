...
[nStrobes_2,Strobe_Times_2, Strobe_Values_2] = plx_event_ts([file_path,filename], 1);
...
varlist = who;
sigs = varlist(strmatch('AD',varlist));
...
Zap_times = floor(Strobe_Times_2*1000);

for s = 1:length(sigs)
    for z = 1:length(Zap_times)-1
        avSig(z,1:701,s) = sig(s,Zap_times(z)-200:Zap_times(z)+500);
    end
end
...
figure
subplot(3,1,1)
plot(-200:500,nanmean(avSig(3,:,1)))
xlim([-200 500])
axis ij
sc = ylim;
xlabel('Time (ms)')
