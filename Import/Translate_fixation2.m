...
%FOR USE ON SESSIONS WITH FIXATION TRIALS EMBEDDED WITH OTHER TRIAL TYPES
...
%limit analysis to Fixation task
x = find(Strobe_Values == 3003);
y = Strobe_Values(x+1);
z = find(Strobe_Values(x+1) == 0);
start = x(z(1));

Strobe_Times = Strobe_Times(start:end);
Strobe_Values = Strobe_Values(start:end);
