...
function [solution minval] = iLBA_sim_vincentized_fixed_b_Neural_Input(file,PopN,GenLim,nTrials,seed,sd_rate,start_param,minimizer)
...
load(file)
load('/Volumes/Dump/Analyses/SAT/Trial_Trial_data/S_Med_NoMed_VisMove_Move_allTrials.mat')
...
if nargin < 7
    ...
    [b T0.slow T0.med T0.fast leakage] = disperse(start_param);
...
initRange = [init.b init.T0 init.leakage];


param = [b T0.slow T0.med T0.fast leakage];
lower = [lb.b lb.T0 lb.leakage];
upper = [ub.b ub.T0 ub.leakage];
...
if minimize
    ...
    if strmatch(minimizer,'simplex')
        ...
        %[solution minval exitflag output] = fminsearch(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_b_Neural_Input(param,ntiles,trls,obs_freq,seed),param,options);
        [solution minval exitflag output] = fminsearchbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_b_Neural_Input(param,ntiles,trls,obs_freq,seed),param,lower,upper,options);
        
        %options = optimset('MaxIter',100000,'MaxFunEvals',100000,'TolFun',1e-20,'TolCon',1e-20);
        %[solution minval exitflag output] = fmincon(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_b_Neural_Input(param,ntiles,trls,obs_freq,seed),param,[],[],[],[],lower,upper,@nonlcon,options);
    ...
    elseif strmatch(minimizer,'ga')
        ...
        [solution minval exitflag output] = ga(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_b_Neural_Input(param,ntiles,trls,obs_freq,seed),numel(param),[],[],[],[],lower,upper,[],options);
    ...
    elseif strmatch(minimizer,'sa')
        ...
        [solution minval exitflag output] = simulannealbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_b_Neural_Input(param,ntiles,trls,obs_freq,seed),param,lower,upper,options);
...
if plotFlag
    ...
    [b T0.slow T0.fast leakage] = disperse(solution);
    %b = b * 100;
    ...
    correct.slow(1:nTr,1) = NaN;
    correct.med(1:nTr,1) = NaN;
    correct.fast(1:nTr,1) = NaN;
    
        
    act1.slow(1:nTr,1:601) = 0;
    act1.med(1:nTr,1:601) = 0;
    act1.fast(1:nTr,1:601) = 0;
    act2.slow(1:nTr,1:601) = 0;
    act2.med(1:nTr,1:601) = 0;
    act2.fast(1:nTr,1:601) = 0;
    
    for n = 1:nTr
    
    Samp1 = randperm(size(allwf_targ.in.slow_correct_made_dead,1),50);
    Samp2 = randperm(size(allwf_targ.in.med_correct,1),50);
    Samp3 = randperm(size(allwf_targ.in.fast_correct_made_dead_withCleared,1),50);
    Samp4 = randperm(size(allwf_targ.out.slow_errors_made_dead,1),50);
    Samp5 = randperm(size(allwf_targ.out.med_errors,1),50);
    Samp6 = randperm(size(allwf_targ.out.fast_errors_made_dead_withCleared,1),50);
    
    
    
    actFun.slow.correct = nanmean(allwf_targ.in.slow_correct_made_dead(Samp1,100:end));
    actFun.med.correct = nanmean(allwf_targ.in.med_correct(Samp2,100:end));
    actFun.fast.correct = nanmean(allwf_targ.in.fast_correct_made_dead_withCleared(Samp3,100:end));
    actFun.slow.errors = nanmean(allwf_targ.out.slow_errors_made_dead(Samp4,100:end));
    actFun.med.errors = nanmean(allwf_targ.out.med_errors(Samp5,100:end));
    actFun.fast.errors = nanmean(allwf_targ.out.fast_errors_made_dead_withCleared(Samp6,100:end));

    
    leakage = 0;
    
    for t =2:1001
        act1.slow(n,t) = act1.slow(n,t-1) + actFun.slow.correct(t) - (leakage * act1.slow(n,t-1));
        act1.med(n,t) = act1.med(n,t-1) + actFun.med.correct(t) - (leakage * act1.med(n,t-1));
        act1.fast(n,t) = act1.fast(n,t-1) + actFun.fast.correct(t) - (leakage * act1.fast(n,t-1));
        act2.slow(n,t) = act2.slow(n,t-1) + actFun.slow.errors(t) - (leakage * act2.slow(n,t-1));
        act2.med(n,t) = act2.med(n,t-1) + actFun.med.errors(t) - (leakage * act2.med(n,t-1));
        act2.fast(n,t) = act2.fast(n,t-1) + actFun.fast.errors(t) - (leakage * act2.fast(n,t-1));
    end
    
%     plot(-100:500,act1.slow,'r',-100:500,act2.slow,'--r',-100:500,act1.med,'k', ...
%         -100:500,act2.med,'--k',-100:500,act1.fast,'g',-100:500,act2.fast,'--g')
%     pause
%     cla
    rt.slow(n,1) = min([find(act1.slow(n,:) > b,1,'first') find(act2.slow(n,:) > b,1,'first')]);
    rt.med(n,1) = min([find(act1.med(n,:) > b,1,'first') find(act2.med(n,:) > b,1,'first')]);
    rt.fast(n,1) = min([find(act1.fast(n,:) > b,1,'first') find(act2.fast(n,:) > b,1,'first')]);

    if (find(act1.slow(n,:)>b,1,'first')) < (find(act2.slow(n,:)>b,1,'first')); correct.slow(n,1) = 1; end
    if (find(act1.med(n,:)>b,1,'first')) < (find(act2.med(n,:)>b,1,'first')); correct.med(n,1) = 1; end
    if (find(act1.fast(n,:)>b,1,'first')) < (find(act2.fast(n,:)>b,1,'first')); correct.fast(n,1) = 1; end
...
%     mult_mat = repmat(linspace(1,size(cross1.slow,2),size(cross1.slow,2)),nTr,1);
%     
...
%     
    ...
    rt.slow = rt.slow + T0.slow ; %correct for diff operation earlier;
