...
function [solution minval] = iLBA_sim_vincentized_fixed_A(file,PopN,GenLim,nTrials,seed,start_param,minimizer)
...
outdir = '/volumes/Dump/Analyses/SAT/Models/iLBA_X2/fixed_A/';
%outdir = '//scratch/heitzrp/Output/iLBA/fixed_A/';

if nargin < 7; minimizer = 'simplex'; end
...
if nargin < 6
    ...
    [A b.slow b.med b.fast v.slow v.med v.fast T0.slow T0.med T0.fast leakage sd_rate] = disperse(start_param);
...
param = [A b.slow b.med b.fast v.slow v.med v.fast T0.slow T0.med T0.fast leakage sd_rate];
...
if minimize
    ...
    if strmatch(minimizer,'simplex')
        ...
        %[solution minval exitflag output] = fminsearch(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_A(param,ntiles,trls,obs_freq,seed,simBound),param,options);
        [solution minval exitflag output] = fminsearchbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_A(param,ntiles,trls,obs_freq,seed,simBound),param,lower,upper,options);
    ...
    elseif strmatch(minimizer,'ga')
        ...
        [solution minval exitflag output] = ga(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_A(param,ntiles,trls,obs_freq,seed,simBound),numel(param),[],[],[],[],lower,upper,[],options);
...
if plotFlag
    ...
    [A b.slow b.med b.fast v.slow v.med v.fast T0.slow T0.med T0.fast leakage sd_rate] = disperse(solution);
    b.slow = b.slow * 100;
    b.med = b.med * 100;
    b.fast = b.fast * 100;
    ...
    %============
    % C++ CODE
    act1.slow = leaky_integrate2(rate1.slow',start1,leakage)';
    act1.med = leaky_integrate2(rate1.med',start1,leakage)';
    act1.fast = leaky_integrate2(rate1.fast',start1,leakage)';
    
    act2.slow = leaky_integrate2(rate2.slow',start2,leakage)';
    act2.med = leaky_integrate2(rate2.med',start2,leakage)';
    act2.fast = leaky_integrate2(rate2.fast',start2,leakage)';
