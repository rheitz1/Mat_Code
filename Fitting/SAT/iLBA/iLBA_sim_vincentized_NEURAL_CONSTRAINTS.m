...
function [solution minval] = iLBA_sim_vincentized_NEURAL_CONSTRAINTS(file,PopN,GenLim,nTrials,seed,sd_rate,start_param,minimizer)
...
lb.b = [80];
lb.v = [.51 .51 .51];
lb.T0 = [T0.slow - 5 T0.med - 5 T0.fast - 5];
lb.leakage = [.005];
%lb.sd_rate = .1;
...
ub.b = [400];
ub.v = [.95 .95 .95];
ub.T0 = [T0.slow + 5 T0.med + 5 T0.fast+5];
ub.leakage = [.05];
%ub.sd_rate = .4;
...
if minimize
    ...
    if strmatch(minimizer,'simplex')
        ...
        %[solution minval exitflag output] = fminsearch(@(param) iLBA_sim_leaky_integrator_calc_X2_NEURAL_CONSTRAINTS(param,ntiles,trls,obs_freq,seed,simBound),param,options);
        [solution minval exitflag output] = fminsearchbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_NEURAL_CONSTRAINTS(param,ntiles,trls,obs_freq,seed,simBound),param,lower,upper,options);
        
        %options = optimset('MaxIter',100000,'MaxFunEvals',100000,'TolFun',1e-20,'TolCon',1e-20);
        %[solution minval exitflag output] = fmincon(@(param) iLBA_sim_leaky_integrator_calc_X2_NEURAL_CONSTRAINTS(param,ntiles,trls,obs_freq,seed,simBound),param,[],[],[],[],lower,upper,@nonlcon,options);
    ...
    elseif strmatch(minimizer,'ga')
        ...
        [solution minval exitflag output] = ga(@(param) iLBA_sim_leaky_integrator_calc_X2_NEURAL_CONSTRAINTS(param,ntiles,trls,obs_freq,seed,simBound),numel(param),[],[],[],[],lower,upper,[],options);
    ...
    elseif strmatch(minimizer,'sa')
        ...
        [solution minval exitflag output] = simulannealbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_NEURAL_CONSTRAINTS(param,ntiles,trls,obs_freq,seed,simBound),param,lower,upper,options);
...
%     cross2.fast(force2.fast,size(cross2.fast,2)) = 1;
    ...
    if truncSim
        ...
        disp(['Removing ' mat2str(length(find(rt.med+T0.med < simBound.med(1)))) ' MED trials']);
        disp(['Removing ' mat2str(length(find(rt.med+T0.med > simBound.med(2)))) ' MED trials']);
