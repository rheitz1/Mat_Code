...
function [solution minval] = iLBA_sim_vincentized_fixed_v(file,PopN,GenLim,nTrials,seed,start_param,minimizer)
...
outdir = '/volumes/Dump/Analyses/SAT/Models/iLBA_X2/fixed_v/';
%outdir = '//scratch/heitzrp/Output/iLBA/fixed_v/';
...
if nargin < 6
    ...
    [A.slow A.med A.fast b.slow b.med b.fast v T0.slow T0.med T0.fast leakage sd_rate] = disperse(start_param);
...
param = [A.slow A.med A.fast b.slow b.med b.fast v T0.slow T0.med T0.fast leakage sd_rate];
...
if minimize
    ...
    if strmatch(minimizer,'simplex')
        ...
        %[solution minval exitflag output] = fminsearch(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_v(param,ntiles,trls,obs_freq,seed,simBound),param,options);
        [solution minval exitflag output] = fminsearchbnd(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_v(param,ntiles,trls,obs_freq,seed,simBound),param,lower,upper,options);
    ...
    elseif strmatch(minimizer,'ga')
        ...
        [solution minval exitflag output] = ga(@(param) iLBA_sim_leaky_integrator_calc_X2_fixed_v(param,ntiles,trls,obs_freq,seed,simBound),numel(param),[],[],[],[],lower,upper,[],options);
...
if plotFlag
    ...
    [A.slow A.med A.fast b.slow b.med b.fast v T0.slow T0.med T0.fast leakage sd_rate] = disperse(solution);
    b.slow = b.slow * 100;
    b.med = b.med * 100;
    b.fast = b.fast * 100;
    ...
    rate1.slow = abs(normrnd(v,sd_rate,nTrials,1));
    rate1.med = abs(normrnd(v,sd_rate,nTrials,1));
    rate1.fast = abs(normrnd(v,sd_rate,nTrials,1));
    
    rate1.slow(find(rate1.slow > 1)) = .9999;
    rate1.med(find(rate1.med > 1)) = .9999;
    rate1.fast(find(rate1.fast > 1)) = .9999;
    
    rate2.slow = abs(normrnd(1-v,sd_rate,nTrials,1));
    rate2.med = abs(normrnd(1-v,sd_rate,nTrials,1));
    rate2.fast = abs(normrnd(1-v,sd_rate,nTrials,1));
