...
[A.slow A.med A.fast b v.slow v.med v.fast T0.slow T0.med T0.fast] = disperse(state.Population(best,:));
...
linear1.slow = cumsum([zeros(nTrials,1) repmat(rate1.slow,1,2000)],2) + repmat(start1.slow,1,2001);
linear1.med = cumsum([zeros(nTrials,1) repmat(rate1.med,1,2000)],2) + repmat(start1.med,1,2001);
linear1.fast = cumsum([zeros(nTrials,1) repmat(rate1.fast,1,2000)],2) + repmat(start1.fast,1,2001);

linear2.slow = cumsum([zeros(nTrials,1) repmat(rate2.slow,1,2000)],2) + repmat(start2.slow,1,2001);
linear2.med = cumsum([zeros(nTrials,1) repmat(rate2.med,1,2000)],2) + repmat(start2.med,1,2001);
linear2.fast = cumsum([zeros(nTrials,1) repmat(rate2.fast,1,2000)],2) + repmat(start2.fast,1,2001);
