function [TrialStart_,Strobe_Times] = osx_EventTranslator_SAT_longBase_noChunk(file_path,filename,TrialStart_TimeStamps,outfile,target_on_Evt2,fix_on_Evt2,Strobe_Times,Strobe_Values)
...
%Initialize Variables
        TrialStart_ = NaN(nTrials,2);
        Errors_ = NaN(nTrials,7);
        Correct_ = NaN(nTrials,2);
        Decide_ = NaN(nTrials,2);
        Target_ = NaN(nTrials,14);
        SaccDir_ = NaN(nTrials,1);
        Stimuli_ = NaN(nTrials,7);
        MStim_ = NaN(nTrials,3);
        Gains_EyeX = NaN(nTrials,1);
        Gains_EyeY = NaN(nTrials,1);
        Gains_XYF = NaN(nTrials,1);
        Gains_YXF = NaN(nTrials,1);
        SAT_ = NaN(nTrials,12);
        FixOn_ = NaN(nTrials,1);
        FixTime_Jit_ = NaN(nTrials,1);
        FixAcqTime_ = NaN(nTrials,1);
        BellOn_ = NaN(nTrials,1);
        JuiceOn_ = NaN(nTrials,1);
        MG_Hold_ = NaN(nTrials,1);
...
%     nTrials = curCHUNK*maxTrialNo+NEXTSTEP;
    ...
    for i = 1:nTrials
        ...
        if i > 1 && i < nTrials;            TrialEvents_AcquireTime = find(Strobe_Times >= TrialStart_TimeStamps(i-1) & Strobe_Times < TrialStart_TimeStamps(i+1));        end
        
        %if last trial
        if i == nTrials;            TrialEvents_AcquireTime = find(Strobe_Times >= TrialStart_TimeStamps(i-1));        end
