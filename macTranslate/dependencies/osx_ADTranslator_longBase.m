function [] = osx_ADTranslator_longBase(file_path,filename,TrialStart_,TimeStamps_Strobe_Events,outfile)
...
%     end
    ...
    if size(dat.ContinuousChannels(ich).Values,1) > 0
       ...
       %
       % NOTE: Conversion factor is (1/4096) for gain of 10; or (1/409.6) for gain of 1
       ...
       save([file_path outfile],cell2mat(adID(ich)),'-append','-mat')
       disp ([adID{ich},'....SAVED TO....',[file_path ,  outfile] ])
       eval(['clear ', adID{ich},' adfreq nad tsad fnad'])
       numads = numads + 1;
