...
function [varargout]=p_sacdet(Filename)

%get data and set parameter
q='''';c=',';qcq=[q c q];
PLOTS='np'; % 'p' == display saccades, 'np' == don't plot
SAVEVARS=1;
global EyeX_ EyeY_ EmStart_ Eot_ 
VARS2LOAD={'EyeX_','EyeY_','EmStart_','Eot_'};
for ii=1:size(VARS2LOAD,2)
   eval(['load(',q,Filename,qcq,VARS2LOAD{ii},qcq,'-mat',q,')'])
end
...
%maxros is the number of trials.
...
for ii=0:multifileNo
   ...
   for trl=1:maxTrialNo
      % P_Eot is Eot value from PDP file
   ...
   for trl=1:maxTrialNo
         ...
         %set(gca,'ylim',[-20 max(Thresh_XY(trl,:))])
         %pause
      temp=[];nsac=[];t=[];ncols=[];
   ...
   if (PLOTS=='p')%&(ii==3)
      ...
      for plts=1:maxros
         ...
         for sac=1:Nsac
            ...
            axis(axisv);
            plot([minX:maxX],I_x(plts,minX:maxX),'b.');
            hold on
            plot([minX:maxX],I_y(plts,minX:maxX),'r.');
            ...
            plot([SaccBegin(plts,sac) SaccBegin(plts,sac)],[-maxxy maxxy],'m-')
            plot([SaccEnd(plts,sac) SaccEnd(plts,sac)],[-maxxy maxxy],'k-.')
            drawnow
            hold off
            pause
         end
